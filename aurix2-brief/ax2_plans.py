...
PROJECTS = ('Арбат 2', 'Резиденция Омега')
SLUG = {'Арбат 2': 'arbat', 'Резиденция Омега': 'omega'}
...
if __name__ == '__main__':
    ...
    lots = json.load(open(os.path.join(HERE, 'ax2_lots.json')))
    ...
    for i, x in enumerate(sel, 1):
        ...
        time.sleep(0.3)
    json.dump(sel, open(os.path.join(HERE, 'ax2_msk.json'), 'w'), ensure_ascii=False, indent=1)
