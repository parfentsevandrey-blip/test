"""Cost of one move-in-ready square metre in the PHANTOM location, ₽/м².
...
C_FLAT, C_FIT = (74, 123, 200), (185, 130, 47)
BANDBG = (245, 242, 237)

D = json.load(open('ph_cmp.json'))
FIT = D['fit']
SUB = {'ФАНТОМ': 'делюкс · авторская отделка · 2027',
       'Turgenev': 'элит · сдан 2023 · с отделкой',
       'Николь': 'элит · 2028 · с отделкой',
       'Zvonarsky Deluxe': 'элит · сдан 2019 · вторичка',
       'Дом Франка': 'элит · 2026 · без отделки'}
ROWS = sorted(((n, v[0], v[1]) for n, v in D['ready'].items()), key=lambda r: -(r[1] + r[2]))

img = Image.new('RGB', (W, H), SURFACE); dr = ImageDraw.Draw(img)
L, R, T = 330 * S, 90 * S, 178 * S
B = H - 96 * S
PW = W - L - R
MAXV = 3_500_000
...
dr.text((44 * S, 62 * S), 'У проектов с готовой отделкой — цена как есть. У «Дома Франка», который продаётся',
        font=fnt(17 * S), fill=MUTED)
dr.text((44 * S, 88 * S), f'без отделки, к цене добавлена отделка. Сопоставимая выборка 60–200 м², Циан, 28.07.2026.',
        font=fnt(17 * S), fill=MUTED)
...
ROW = (B - T) / len(ROWS)
BAR = 34 * S
for i, (name, base, fit) in enumerate(ROWS):
    cy = T + ROW * (i + 0.5)
    if name == 'ФАНТОМ':
        dr.rounded_rectangle([44 * S, cy - ROW / 2 + 5 * S, W - 44 * S, cy + ROW / 2 - 5 * S], 8 * S, fill=BANDBG)
    dr.text((44 * S, cy - 22 * S), name, font=fnt(21 * S, name == 'ФАНТОМ'), fill=INK)
    dr.text((44 * S, cy + 4 * S), SUB[name], font=fnt(15 * S), fill=MUTED)
    y0 = cy - BAR / 2
...
img.save('ph_chart.png', quality=95)
