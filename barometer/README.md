...
|---|---|
| `ANTHROPIC_API_KEY` | Включает **глубокий анализ Claude**: оценка 0–100 и обоснование, которые смешиваются с правиловым счётом. Без ключа работает только правиловый слой. |
| `BAROMETER_LLM_MODEL` | Модель Claude (по умолчанию `claude-sonnet-4-6`). |
| `X_BEARER_TOKEN` | Включает **реальную** выгрузку из X/Twitter API v2 (иначе используется `data/samples/twitter.json`). |
...
│   ├── llm.py             Опциональный анализ через Claude
