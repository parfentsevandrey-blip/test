...
3. **Военные аналитики и соцсети** — Telegram (публичный веб-превью `t.me/s/…`,
   без ключей) и X/Twitter через **реверс-инжиниринг** внутреннего веб-эндпоинта
   (`syndication.twitter.com` отдаёт твиты профиля в блоке `__NEXT_DATA__` —
   публичные данные, без ключа). Работает с обычных (резидентных) IP; с
   дата-центровых IP (GitHub Actions, облако) X отдаёт 429 → откат на сэмпл.
   Реальные твиты получаются при запуске сборщика локально. Telegram-канал
   аналитиков при этом всегда реальный.
