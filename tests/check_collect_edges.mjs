...
// ===========================================================================
// isHealthWarn — ОТНОСИТЕЛЬНЫЙ порог: retries/requests > 0.15. Чем длиннее
// удачный прогон, тем надёжнее он маскирует абсолютный отказ в конце.
async function testSilentLoss() {
  ...
  gap(!tail.warn,
    () => `потеряно ${1000 - tail.rows} лотов, а предупреждения нет: порог относительный — ` +
      `${tail.health.retries} ретрая на ${tail.logical} страниц = ${(tail.health.retries / tail.logical).toFixed(3)} ≤ 0.15. ` +
      `Файл уйдёт без суффикса «_проверить», охват ${tail.coverage}%. Чем длиннее сегмент, тем надёжнее прячется отказ в его конце`,
    "обрыв сегмента фиксируется в health независимо от длины прогона");

  const head = await run("тот же обрыв, но на 3-й странице", {
    base: narrow, handler: (f) => (f.page === 3 ? { throw: true } : serve({ offers: band })(f)),
  });
  check(head.warn && !tail.warn,
    `контроль порога: ровно тот же отказ в НАЧАЛЕ прогона предупреждение даёт (${head.health.retries}/${head.logical}), ` +
    "а в конце — нет; заметность потери зависит не от её размера, а от места",
    "порог isHealthWarn ведёт себя одинаково в начале и в конце прогона");
