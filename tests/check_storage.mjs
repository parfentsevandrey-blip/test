...
// 9. Выводы по журналу
// ===========================================================================
// Журнал заводился ради шести вопросов, и его ценность ровно в ответах на них.
// Главное здесь — что «нет данных» НЕ выдаётся за «нет»: отсутствие отказов не
// означает «Retry-After не читается», и путать эти два ответа хуже, чем молчать.
function testVerdicts() {
  section("Выводы по журналу: что он отвечает и когда честно молчит");

  const { api } = bench();
  const V = (roll) => {
    const out = {};
    api.journalVerdicts(roll, api.CONFIG).forEach((v) => { out[v.q] = v; });
    return out;
  };
  const RA = "Читается ли заголовок Retry-After";
  const N429 = "Бывает ли на этом API код 429";
  const LAT = "Типичная задержка ответа";
  const WAF = "Встречалась ли проверка браузера или блокировка";
  const PACE = "Соблюдался ли темп";
  const GAP = "Где закончил адаптивный темп";

  // --- Свёртка по нескольким прогонам: статистика, а не последний прогон.
  const roll = api.rollupRuns([
    { http: 10, pages: 10, byStatus: { 200: 10 }, p50: 100, p95: 200, zeroGaps: 0, raSeen: 0, pacerFinal: 2000 },
    { http: 6, pages: 4, byStatus: { 200: 4, 429: 2 }, p50: 300, p95: 9000, zeroGaps: 0, raSeen: 2, pacerFinal: 8000 },
  ]);
  eq(roll.runs, 2, "прогонов в свёртке");
  eq(roll.http, 16, "обращений суммарно");
  eq(roll.byStatus["429"], 2, "статусы просуммированы");
  eq(roll.p95, 9000, "по p95 берётся ХУДШЕЕ из прогонов, а не среднее");
  eq(roll.p50, 300, "по p50 — медиана прогонов");

  // --- Отказов не было: честное «нет данных», а не «не читается».
  {
    const v = V(api.rollupRuns([{ http: 40, pages: 40, byStatus: { 200: 40 }, p50: 120, p95: 260, zeroGaps: 0, raSeen: 0, pacerFinal: 2000 }]));
    eq(v[RA].a, "нет данных", "Retry-After при нулевых отказах");
    check(/проверять было не на чем/.test(v[RA].note),
      `и объясняет почему: «${v[RA].note}»`, `объяснение отсутствует: «${v[RA].note}»`);
    eq(v[N429].a, "не встречался", "429 при 40 чистых обращениях");
    eq(v[WAF].a, "нет", "блокировок не было");
    eq(v[PACE].a, "да", "темп соблюдён");
  }

  // --- Отказы были, а заголовок не прочитан ни разу: это уже ОТВЕТ.
  {
    const v = V(api.rollupRuns([{ http: 20, pages: 12, byStatus: { 200: 12, 429: 5, NET: 3 }, p50: 200, p95: 900, zeroGaps: 0, raSeen: 0, pacerFinal: 4000 }]));
    eq(v[RA].a, "НЕТ", "Retry-After: 8 отказов и ни одного прочтения");
    check(/CORS-safelist/.test(v[RA].note),
      "и назвал вероятную причину — заголовок вне CORS-safelist",
      `причина не названа: «${v[RA].note}»`);
    eq(v[N429].a, "да, 5", "429 существует на этом эндпоинте");
  }

  // --- Заголовок читается: ветка «уважаем просьбу сервера» жива.
  {
    const v = V(api.rollupRuns([{ http: 10, pages: 6, byStatus: { 200: 6, 429: 4 }, p50: 200, p95: 400, zeroGaps: 0, raSeen: 4, pacerFinal: 4000 }]));
    eq(v[RA].a, "да", "Retry-After читается");
    check(/4 раз из 4/.test(v[RA].note), `с числами: «${v[RA].note}»`, `без чисел: «${v[RA].note}»`);
  }

  // --- Латентность: порог «проверки браузера» из документации Циан.
  {
    const fast = V(api.rollupRuns([{ http: 10, byStatus: { 200: 10 }, p50: 150, p95: 400, zeroGaps: 0, raSeen: 0, pacerFinal: 2000 }]));
    check(/базовая линия/.test(fast[LAT].note), "быстрые ответы объявлены базовой линией", `неверный вывод: «${fast[LAT].note}»`);
    const slow = V(api.rollupRuns([{ http: 10, byStatus: { 200: 10 }, p50: 150, p95: 12000, zeroGaps: 0, raSeen: 0, pacerFinal: 2000 }]));
    check(/проверка браузера/.test(slow[LAT].note),
      `медленные — поводом присмотреться: «${slow[LAT].a}»`, `порог не сработал: «${slow[LAT].note}»`);
  }

  // --- Нулевой зазор — это дефект, а не настройка: пацер где-то не отработал.
  {
    const v = V(api.rollupRuns([{ http: 30, byStatus: { 200: 30 }, p50: 100, p95: 200, zeroGaps: 7, raSeen: 0, pacerFinal: 2000 }]));
    check(/^НЕТ/.test(v[PACE].a), `нулевые зазоры названы прямо: «${v[PACE].a}»`, `нулевые зазоры пропущены: «${v[PACE].a}»`);
    check(/дефект/.test(v[PACE].note), "и помечены как дефект", `не помечены: «${v[PACE].note}»`);
  }

  // --- Упор в потолок = стартовый темп слишком дерзкий.
  {
    const at = V(api.rollupRuns([{ http: 30, byStatus: { 200: 20, 429: 10 }, p50: 100, p95: 200, zeroGaps: 0, raSeen: 0, pacerFinal: api.CONFIG.pacer.ceil }]));
    check(/слишком дерзк/.test(at[GAP].note), `упор в потолок назван: «${at[GAP].note}»`, `упор в потолок не замечен: «${at[GAP].note}»`);
    const calm = V(api.rollupRuns([{ http: 30, byStatus: { 200: 30 }, p50: 100, p95: 200, zeroGaps: 0, raSeen: 0, pacerFinal: api.CONFIG.pacer.start }]));
    check(/запас есть/.test(calm[GAP].note), "спокойный прогон — «запас есть»", `неверный вывод: «${calm[GAP].note}»`);
  }

  // --- Пустой буфер не должен ничего выдумывать.
  {
    const v = V(api.rollupRuns([]));
    eq(v[RA].a, "нет данных", "пустой буфер: Retry-After");
    eq(v[N429].a, "нет данных", "пустой буфер: 429");
    eq(v[LAT].a, "нет данных", "пустой буфер: задержка");
    eq(v[GAP].a, "нет данных", "пустой буфер: темп");
  }
}

// ===========================================================================
// 10. Миграция localStorage -> IndexedDB
// ===========================================================================
...
const BREAKAGES = [
  ...
  {
    // Самый вредный из возможных дефектов вывода: выдать «нет данных» за «нет».
    // Отсутствие отказов не означает, что Retry-After не читается, и решение о
    // редизайне пацера на таком «ответе» было бы принято по пустому месту.
    name: "«нет данных» выдаётся за отрицательный ответ",
    from: `    out.push(fails === 0`,
    to: `    out.push(false`,
    expect: "Retry-After при нулевых отказах",
  },
  {
