...
# Расстояния от точки между домами — по прямой, OpenStreetMap.
DISTRICT = [
    ['Площадь района', '2,11 км² (276 га)'],
    ['Население, 2025', '36 942 человека'],
    ['Плотность', '17 508 чел. на км²'],
    ['Зелёные зоны', '21 га и 19 скверов'],
    ['Станций метро в районе', '6'],
    ['Границы', 'от Кремля до Садового кольца, запад ЦАО'],
    ['Что за окном', 'зона охраны Кремля'],
    ['Ведомства в районе', 'Минобороны, МИД, Минэкономразвития'],
]

DIST_PRO = [
    ['Шесть станций метро на 2,11 км² территории',
     'четыре из них — «Арбатская», «Библиотека имени Ленина», «Боровицкая» '
     'и «Александровский сад» — связаны переходами в один узел в 251 метре '
     'от домов. Из него доступны четыре линии метро.'],
    ['Первая линия достопримечательностей начинается через дорогу',
     'Дом Пашкова и Российская государственная библиотека — 240 метров, '
     'ГМИИ имени Пушкина — 453, Кремль и Александровский сад — 986, '
     'Большой театр — 1 265.'],
    ['Зона охраны Кремля ограничивает новую застройку',
     'плотной новой стройки в этих кварталах не будет: за год в километре '
     'вокруг набралось 84 строящихся лота, и 43 из них — «Кло 17» и «Люче».'],
    ['Предложение штучное и почти всё вторичное',
     'из 268 лотов когорты дороже 700 тыс. ₽ за метр 184 — в готовых домах, '
     'где продаётся по три-тринадцать квартир. Массового выбора в локации нет.'],
    ['Пешеходная среда сложилась и не меняется',
     'улица Арбат, Гоголевский бульвар, Знаменка и Воздвиженка образуют '
     'связный пешеходный контур; на 2,11 км² приходится 19 скверов.'],
]

DIST_CONTRA = [
    ['Плотность 17 508 человек на км² — 47-е место среди районов Москвы',
     'при этом свободных участков нет: любой новый проект встаёт на место '
     'снесённого или реконструируемого здания.'],
    ['Административная функция района',
     'Минобороны на Знаменке, МИД на Смоленской, Минэкономразвития — '
     'режимные объекты, перекрытия движения и охраняемые периметры '
     'в шаговой доступности.'],
    ['Зелени мало по городским меркам',
     '21 гектар на 2,11 км² — около десятой части территории. Ближайший '
     'полноценный парк, Александровский сад, работает как городская '
     'достопримечательность, а не как двор.'],
    ['Туристический поток на улице Арбат',
     'пешеходная улица начинается в 400 метрах от домов и остаётся главным '
     'туристическим маршрутом центра со всем, что к нему прилагается.'],
    ['Три стройки одновременно в километре',
     '«Кло 17», «Люче» и «Никитский-6» в 356 метрах строятся параллельно; '
     'у «Люче» под окнами ещё и археологические работы с усилением '
     'соседней застройки.'],
]

# ── застройщик ──────────────────────────────────────────────────────────────
# Данные о компании — из открытых источников: Metrium (выручка 2024),
# карточки застройщика, публикации «Ведомостей» и «Новострой-М».
BUILDER = [
    ['Компания', 'MR Group, на рынке с 2003 года'],
    ['Элитное направление', 'MR Private'],
    ['Портфель', '52 объекта, 11,8 млн м²'],
    ['Активные проекты', 'свыше 2,6 млн м²'],
    ['Выручка от продаж квартир, 2024', '128,8 млрд ₽ (расчёт Metrium)'],
    ['Позиция на рынке', 'топ-3 Москвы, топ-7 России'],
    ['Архитектор «Кло 17»', 'Paris Classical Architecture'],
    ['Архитектор «Люче»', 'Archea Associati, Марко Казамонти'],
    ['Генподрядчик «Люче»', 'ООО «Сварго»'],
    ['Конструктив', 'монолит-кирпич в обоих домах'],
]

# ── хронология ──────────────────────────────────────────────────────────────
TIMELINE = [
    ['Октябрь 2021', 'MR Group объявляет о новом элитном проекте в Крестовоздвиженском переулке'],
    ['Декабрь 2023', 'На участке «Люче» завершён демонтаж надземной части здания'],
    ['2024 – 2025', 'Нулевой цикл «Люче»: котлован, археологические исследования, усиление соседней застройки, перенос коммуникаций'],
    ['2 сентября 2025', 'На Циан открыт 21 лот «Люче» из 26, которые в продаже сегодня'],
    ['16 сентября 2025', 'На Циан открыты 16 лотов «Кло 17» из 17'],
    ['Апрель 2026', 'Добавлены последние лоты обоих домов; цены с момента публикации не менялись'],
    ['III квартал 2026', 'Заявленный в объявлениях срок сдачи обоих домов'],
    ['II квартал 2027', 'Выдача ключей «Люче» по карточке проекта'],
]

# ── что определяет продукт ──────────────────────────────────────────────────
FEATURES = [
    ['64 метра между двумя домами одного застройщика',
     'два клубных проекта MR Group выходят на один квартал между Воздвиженкой '
     'и Знаменкой. Локация у них общая; выбор покупателя идёт между двумя '
     'подъездами в ста шагах друг от друга.'],
    ['Потолки 3,2 – 4,65 м',
     'верхняя отметка московского элитного сегмента: типовой ориентир — '
     '3,2–3,5 м. В «Люче» карточки называют 3,9–4,65 м, в «Кло 17» — '
     '3,2 м на основных этажах и 3,75 на седьмом.'],
    ['93 машино-места на 72 резиденции',
     '64 места на 46 квартир в «Люче» и 29 на 26 в «Кло 17» — 1,39 и 1,12 '
     'места на квартиру. Внутри Бульварного кольца улица парковку не заменяет.'],
    ['Лоты без отделки',
     'к бюджету покупки добавляется ремонт. В элитном сегменте это '
     '300–500 тыс. ₽ за метр, то есть 45–75 млн ₽ на среднюю резиденцию '
     '«Кло 17» и 61–101 млн ₽ на средний лот «Люче».'],
    ['251 метр до узла из четырёх станций метро',
     '«Арбатская», «Библиотека имени Ленина», «Боровицкая» и «Александровский '
     'сад» связаны переходами: четыре линии из одной точки.'],
    ['Верхние уровни в открытую продажу не выведены',
     'карточки описывают площади до 615 и 706 м², в объявлениях верхняя '
     'точка — 359 м². Пентхаусы продают вне открытой экспозиции.'],
]

# ── что даёт один и тот же бюджет в каждом доме ─────────────────────────────
# Для четырёх бюджетов подбирается ближайший по цене лот в каждом доме.
BUDGETS = [250_000_000, 400_000_000, 550_000_000, 700_000_000]


def nearest(h, budget):
    return min(h['lots'], key=lambda l: abs(l['priceRub'] - budget))
...
for _b in BUDGETS:
    ...
    for h in HOUSES:
        l = nearest(h, _b)
        row.append(f"{l['totalArea']:.0f} м², этаж {l['floor']}")
        row.append(mln(l['priceRub']))
    EQUAL_ROWS.append(row)

# ── что входит в дом ────────────────────────────────────────────────────────
# Состав общих зон и инженерии — из карточек проектов: novostroy-m.ru,
# m2.ru, elitnoe.ru, novostroev.ru. Там, где карточки молчат, так и написано.
INFRA_ROWS = [
    ['Архитектор', 'Paris Classical Architecture', 'Archea Associati, Марко Казамонти'],
    ['Конструктив', 'монолит-кирпич', 'монолит-кирпич'],
    ['Общая площадь', '8,1 тыс. м², из них 4,1 тыс. жилья', 'в карточках не указана'],
    ['Велнес и спа', 'бассейн, спа, сауна, хаммам,\nмассажный кабинет, фитнес-зал',
     'спа-зона и оздоровительный центр'],
    ['Двор', 'приватный сад', 'внутренний парк 0,5 га'],
    ['Лифты', 'в карточках не указано', '8 пассажирских и 8 грузовых'],
    ['Что ещё', 'закрытая территория', 'переговорные, консьерж,\nкоммерческие помещения'],
    ['Генподрядчик', 'в карточках не указан', 'ООО «Сварго»'],
]

# ── полный бюджет резиденции ────────────────────────────────────────────────
# Отделка элитного уровня в Москве — 300–500 тыс. ₽ за метр; в таблице
# взята середина вилки, 400 тыс. ₽. Метр покупки — медиана обоих домов.
FINISH = 400_000
PPM_MED = med(l['ppm'] for l in ALL)
BUDGET_AREAS = [83, 100, 149, 200, 250, 359]
BUDGET = [
    [f'{a}', mln(a * PPM_MED), mln(a * FINISH), mln(a * (PPM_MED + FINISH))]
...
# Сравнение идёт внутри одних и тех же домов — локация, год и класс у обеих
# групп совпадают, различается только заявленное состояние квартиры.
# Медиана берётся по домам, а не по всем лотам сразу: иначе дом с самым
# длинным списком лотов перетягивает результат на себя.
REPAIR_RU = {'design': 'дизайнерский', 'euro': 'евроремонт',
             'cosmetic': 'косметический', 'no': 'без ремонта'}
_rep_path = os.path.join(HERE, 'ac_repair_lots.json')
REPAIR_LOTS = json.load(open(_rep_path, encoding='utf-8')) if os.path.exists(_rep_path) else []

_by_house = {}
for _l in REPAIR_LOTS:
    if _l['repair']:
        _by_house.setdefault(_l['complex'], {}).setdefault(_l['repair'], []).append(_l['ppm'])

FIN_ROWS, _pairs = [], []
for _name, _v in _by_house.items():
    _a = _v.get('design')
    _b = [p for r, ps in _v.items() if r != 'design' for p in ps]
    if not _a or not _b:
        continue
    _ma, _mb = med(_a), med(_b)
    _pairs.append((_name, _ma, _mb))
    _d = (_ma / _mb - 1) * 100
    FIN_ROWS.append([_name, str(len(_a)), nf(round(_ma)), str(len(_b)), nf(round(_mb)),
                     f'{_d:+.0f} %'.replace('+0 %', '0 %').replace('-', '\u2212')])
FIN_ROWS.sort(key=lambda r: -float(r[5].replace(' %', '').replace('+', '')
                                               .replace('\u2212', '-')))

_kinds = {}
for _l in REPAIR_LOTS:
    _kinds[_l['repair']] = _kinds.get(_l['repair'], 0) + 1
KIND_ROWS = [[REPAIR_RU.get(k, 'не указан'), str(v),
              nf(round(med([l['ppm'] for l in REPAIR_LOTS if l['repair'] == k])))]
             for k, v in sorted(_kinds.items(), key=lambda kv: -kv[1])]

FIN = {}
if _pairs:
    _pcts = sorted((a / b - 1) * 100 for _, a, b in _pairs)
    FIN = {
        'houses': len(_pairs),
        'read': len(REPAIR_LOTS),
        ...
        'med': f'{med(_pcts):+.0f}'.replace('+0', '0'),
        'lo': f'{_pcts[0]:+.0f}'.replace('-', '\u2212'), 'hi': f'{_pcts[-1]:+.0f}',
        'above': sum(1 for p in _pcts if p > 10),
        'flat': sum(1 for p in _pcts if -10 <= p <= 10),
        'designMed': nf(round(med([l['ppm'] for l in REPAIR_LOTS if l['repair'] == 'design']))),
    }

_cards_rep = os.path.join(HERE, 'ac_repair_cards.json')
REPAIR_CARDS = json.load(open(_cards_rep, encoding='utf-8')) if os.path.exists(_cards_rep) else []
...
if __name__ == '__main__':
    K = {
        ...
        'kindRows': KIND_ROWS,
        'fin': FIN,
        'repairCards': REPAIR_CARDS,
        'repairCardsMed': nf(round(med(c['ppmNum'] for c in REPAIR_CARDS))) if REPAIR_CARDS else '—',
        'budget': BUDGET,
