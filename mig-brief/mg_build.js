...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      body(`Цен у проекта пока нет: брокерские карточки показывают статус «проектирование» и собирают лист ожидания, старт продаж заявлен на III–IV квартал 2026 года. Поэтому ценовая часть справки построена не на прайсе проекта, а на том, во сколько локация оценивает метр сегодня.`),
      ...
      spacer(250),
      ...bullets([
        [txt(`Земля обошлась в ${N.landPer} ₽ на метр будущего жилья. `), txt(`Стартовая цена лота на торгах — ${N.lotPrice} млрд ₽ на ${N.housing} млн м² жилья. На фоне цены метра в локации — ${C.newMed} ₽ — это доли процента: в проектах КРТ инвестор платит не за землю, а за обязательства по стройке и инфраструктуре.`, { bold: true })],
      ...
      spacer(46),
      ...bullets([
        [txt('Проект сопоставим по объёму с годовым вводом жилья в округе. '), txt('1,4 млн м² — это не дом и не квартал, а полноценная программа застройки, растянутая на четверть века.', { bold: true })],
