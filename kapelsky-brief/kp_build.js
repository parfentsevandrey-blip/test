...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      // ═══════════════ СРЕДНИЕ ПО ДОМАМ ═══════════════
      h1('Средний метр по домам и экономика выхода', { br: true }),
      body(`Строка на дом: сколько лотов в продаже, средний метр и что будет, если среднюю резиденцию «Капельского» в 110 м² продать по цене этого дома. Полный бюджет резиденции — ${K.exit.total} млн ₽: ${K.exit.buy} млн покупка по цене закрытых продаж и ${K.exit.fit} млн ремонт по 400 тыс. ₽ за метр. Дома отсортированы по метру, номера совпадают с картой конкурентов.`, { after: 200 }),
      statTiles([
        [K.exit.buy + ' млн ₽', 'Покупка, 110 м²'],
        [K.exit.fit + ' млн ₽', 'Ремонт'],
        [K.exit.total + ' млн ₽', 'Полный бюджет'],
        [K.exit.ppmTotal + ' ₽', 'Полный метр — точка безубыточности', true],
      ]),
      spacer(230),
      dataTable(
        ['№', 'Дом', 'Тип', 'Лотов', 'Средний\nметр, ₽', 'Цена продажи\n110 м², млн ₽', 'Прибыль,\nмлн ₽', 'Маржа'],
        K.exitRows, [450, 1900, 1400, 700, 1250, 1350, 1300, 1288],
        { boldFirstCol: true, leftCols: [1, 2] },
      ),
      spacer(36),
      note('Цена продажи — средний метр соседнего дома, умноженный на 110 м². Налоги, комиссия брокера и три года до ключей в расчёт не заложены; цены взяты сегодняшние, прогноза на 2029 год здесь нет.'),
      spacer(40),
      ...bullets([
        [txt(`Полный метр получается ${K.exit.ppmTotal} ₽ — это и есть точка безубыточности. `), txt(`Она выше средневзвешенного премиум-метра Москвы (${nfmt(1600000)} ₽ по NF Group) и на 67 % выше медианы средних метров по одиннадцати соседям (1 018 437 ₽).`, { bold: true })],
        [txt(`Выше точки безубыточности в когорте торгуются ${K.exit.over} дома из ${K.exit.all}. `), txt('Продажа по метру «ФАНТОМа» даёт +82,5 млн ₽ и 44 % маржи, по «Форуму» — +54,6 млн и 29 %, по «Дому Франка» — +13,6 млн и 7 %. Все три — премиальные новостройки у «Сухаревской» и «Цветного».', { bold: true })],
        [txt('У остальных восьми домов выручка бюджет не покрывает. '), txt('По метру ЦВЕТ32 выход даёт −37,5 млн ₽, по Barkli Park −75,0 млн, по Sole Hill −92,2 млн, по «Моду» −119,9 млн. Готовая вторичка района на такой бюджет не выходит.', { bold: true })],
        [txt('Маржа целиком зависит от того, в каком ряду дом окажется к 2029 году. '), txt('Сценарий с прибылью требует, чтобы «Капельский» продавался в одном ряду с премиальными новостройками, где метр уже стоит 1,8–2,4 млн ₽. Сценарий вторички района даёт минус на любом из восьми домов.', { bold: true })],
        [txt(`Ремонт в этой арифметике добавляет ${K.exit.fit} млн ₽ к бюджету. `), txt(`Рынок локации возвращает за него ${K.fin.gap} ₽ за метр, то есть около 18 млн ₽ на 110 м² — примерно ${K.fin.coverLo}–${K.fin.coverHi} % вложенного.`, { bold: true })],
        [txt('Чтобы медиана локации дошла до точки безубыточности за три года, метр должен расти на 19 % в год. '), txt('Элитный сегмент Москвы за последний год прибавил 9 % при падении числа сделок на 45 %. Разрыв закрывается либо переоценкой локации, либо тем, что дом продаётся дороже своего района.', { bold: true })],
      ]),
