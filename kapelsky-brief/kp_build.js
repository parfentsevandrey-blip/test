...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      spacer(30),
      note(`«Отделка не указана» означает, что застройщик её не заявил, а не то, что квартира продаётся оболочкой. Медиана метра по трём премиальным проектам (${plural(C.prem, ['лот', 'лота', 'лотов'])}) — ${C.premMed} ₽, по двум высотным бизнес-класса (${plural(C.biz, ['лот', 'лота', 'лотов'])}) — ${C.bizMed} ₽.`),
      ...
      spacer(60),
      note('Фотографии из объявлений о продаже. Дома: ЦВЕТ32 и «Легенда Цветного» у Цветного бульвара, Barkli Park и Sole Hill в 890–960 метрах от участка, Dialog у Красносельской, «Мод» и «Волга» у Сущёвского вала. Все квартиры — вторичка в домах 2010–2024 годов постройки.'),
      spacer(40),
      ...bullets([
        [txt(`Диапазон — от ${K.cards[K.cards.length - 1].ppm} до ${K.cards[0].ppm} ₽ за метр. `), txt(`Верх задают ${K.cards[0].name} и ЦВЕТ32 у Цветного бульвара, низ — «Волга» и «Мод» у Сущёвского вала. Медиана этих двадцати четырёх квартир — ${K.cardsMed} ₽ за метр, то есть заметно ниже 1,3 млн ₽ у «Капельского», причём въехать в них можно сегодня.`, { bold: true })],
        [txt(`Такая же квартира без ремонта в тех же домах стоит на ${K.fin.pct} % дешевле — разница ${K.fin.gap} ₽ за метр. `), txt(`Ремонт этого уровня обходится в 300–500 тыс. ₽ за метр, то есть при перепродаже возвращается ${K.fin.coverLo}–${K.fin.coverHi} % вложенного. Для покупателя оболочки в «Капельском» это значит, что ремонт нужно закладывать как расход на жизнь в квартире, а не как вложение в её цену.`, { bold: true })],
      ]),
