...
const K  = JSON.parse(fs.readFileSync(path.join(__dirname, 'sum_tables.json'), 'utf8'));
...
const CONTENT_W = 15137;           // A4 LANDSCAPE minus 15 mm side margins, in DXA
const MEASURE = 4200;              // right indent for running text on a wide page
...
function dataTable(headers, rows, widths, o = {}) {
  const cell = (text, { bold, align, fill, color, size, w, last, first, top, vtop } = {}) =>
    new TableCell({
      width: { size: w, type: WidthType.DXA },
      shading: fill ? { type: ShadingType.CLEAR, fill, color: 'auto' } : undefined,
      margins: { top: 52, bottom: 52, left: 100, right: 100 },
      verticalAlign: vtop ? VerticalAlign.TOP : VerticalAlign.CENTER,
      borders: {
        top: top || hair(),
        bottom: hair(),
