...
- **512 KB/s, the whole time the tunnel is up.** 0.7.9 warmed the video circuit at 128 KB/s and only while a video played. As asked: the stream now runs at 512 KB/s by default (128, 256 and 1024 to choose from) for as long as the tunnel is up — on the video's own circuit while a YouTube connection is open, on the main route otherwise, which shares the guard or bridge link with everything. "Only while a video plays" remains a choice.
...
- **512 КБ/с всё время, пока туннель поднят.** В 0.7.9 цепочка видео грелась на 128 КБ/с и только пока шло видео. Как просили: теперь поток идёт на 512 КБ/с по умолчанию (на выбор 128, 256 и 1024) всё время, пока туннель поднят — по цепочке самого видео, пока открыто соединение с YouTube, иначе по основному маршруту, который делит звено guard или моста со всем остальным. «Только пока идёт видео» остаётся выбором.
