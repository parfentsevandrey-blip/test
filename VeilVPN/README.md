...
> **Готовый DMG:** <https://github.com/parfentsevandrey-blip/test/releases/tag/veil-v0.7.10>
> Сборка универсальная (Apple silicon + Intel), собирается автоматически
> GitHub Actions на macOS-раннере из этого каталога.

## Возможности

- **Тёплый старт.** Один процесс `tor` держится загруженным, но отключённым от сети
  (`DisableNetwork 1`, без SOCKS-порта — со стороны это неотличимо от невыключенного Tor).
  Нажатие «Подключиться» — одна команда вместо запуска процесса, рукопожатия и перечитывания
  консенсуса. Какой транспорт пробовать первым, решают файл `state` самого Tor (какие входные
  стражи он здесь реально подтвердил) и история подключений в этой сети, а не четырёхсекундная
  проба перед стартом. Каждая попытка контролируется по стадиям по событиям Tor: безнадёжная
  умирает за секунды, а медленная, но рабочая защищена четырьмя «спасателями».
- **Измеряемые цепочки.** Veil держит несколько цепочек открытыми одновременно — каждая
  задаётся своей парой SOCKS-логин/пароль, а `IsolateSOCKSAuth` в Tor гарантирует, что два
  потока с разными парами никогда не разделят цепочку. Цепочки постоянно измеряются, каждое
  новое соединение уходит по быстрой, сайт закрепляется за своей цепочкой (IP выхода не
  меняется посреди сессии), а медленное соединение тихо переигрывается на второй цепочке.
  Уже открытые соединения при этом не трогаются вообще.
- **Раздел «Безопасность».** Один экран отвечает на вопрос «насколько я сейчас защищён»:
  шесть противников со своими потолками, каждая находка с ценой и кнопкой исправления там,
  где честное исправление существует, три пресета с показом будущей оценки и самопроверка,
  которая доказывает заявления, а не повторяет их.
...
- **Честная задержка.** Маршрут измеряется своим замером: SOCKS5-подключение через Tor к
  anycast-адресу, три пробы каждые 25 секунд, показывается медиана последних двенадцати, а
  всплески — отдельной цифрой джиттера. Проверка через check.torproject.org только подтверждает выход.
- **Быстрое пробуждение.** После сна Veil открывает настоящий поток через туннель: если трафик
  идёт, ничего не перезапускается. Проверки сети не дублируются, сброс Wi-Fi не повторяется чаще
  чем раз в 90 секунд, у транспортов ограниченный бюджет, у попытки — общий потолок.
