...
в которую можно заехать» идёт в одну сторону: отделка добавляется конкурентам,
а не нам.

Отделка премиум-класса в Даниловском оценена в 200 тыс. ₽/м² — стоимость
работ и материалов «под ключ»: выше типового бизнес-класса и ниже де-люкса.
"""
import json, sys, os
...
def block(name):
    band = [x for x in L[name] if LO <= x['area'] <= HI and cmp_ok(x)]
    if not band:
