...
У нашего лота отделка от застройщика уже есть, поэтому доплачивать нечего.
У соседей, где экспозиция в бетоне, к цене добавлено 200 тыс. ₽/м².
...
D = json.load(open('hl/hl_tables.json'))
FIT, OUR = D['fit'], D['our']
ROWS = [('НАШ ЛОТ', OUR['ppm'], 0, '125,1 м², 8 этаж · отделка от застройщика')]
for n, (b, f, sub) in D['ready'].items():
    ROWS.append((n if n != 'HIGH LIFE' else 'HIGH LIFE — весь дом', b, f, sub))
ROWS.sort(key=lambda r: -(r[1] + r[2]))

img = Image.new('RGB', (W, H), SURFACE); dr = ImageDraw.Draw(img)
L, R, T = 400 * S, 210 * S, 178 * S
B = H - 96 * S
PW, MAXV = W - L - R, 1_650_000
...
dr.text((44 * S, 62 * S), 'У нашего лота отделка от застройщика уже сделана — доплачивать нечего. Там, где экспозиция соседей', font=fnt(17 * S), fill=MUTED)
dr.text((44 * S, 88 * S), 'идёт в бетоне, к цене метра добавлена отделка. Сопоставимая выборка 110–140 м², Циан, 30.07.2026.', font=fnt(17 * S), fill=MUTED)
...
for v in range(0, MAXV + 1, 250_000):
...
img.save('hl/hl_chart.png', quality=95); print('saved', img.size, 643 / (W / H))
