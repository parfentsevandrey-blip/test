...
только по тому, что видно. Из перечня выводится объём: капитальный /
...
жильё и для кого; что его продаёт или топит; что с ним сделает следующий
...
### Осмотр — раньше признаков

Вывод отвечает на три вопроса, но опирается на признаки, и поэтому
звучит как заключение аналитика. Покупателю нужно другое: пройти квартиру
глазами — с порога, по комнатам, кто и для кого делал, чего не показали,
что он с этим сделает. Это отдельный блок записи `human`, и он пишется
**до** признаков, а не выводится из них: попытка собрать такой текст из
признаков дала опись и была откачена. Как смотреть, как говорить и что
проверяет движок — в [`walkthrough.md`](walkthrough.md).
...
В `marks.json` на каждый лот: `human` (осмотр глазами человека —
первым, см. [`walkthrough.md`](walkthrough.md)), `markers`, `proof`,
`photosSeen` (сколько кадров смотрели), `framesSeen` (номера кадров),
`framesFull` (какие из них открывали в оригинале), `age` (возраст ремонта),
`works` (что менять), `verdict` (итог — обязателен при возрасте) и `note` —
служебные пометки для перепроверки.
