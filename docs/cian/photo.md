...
### Номера кадров и планировки

На контактном листе у каждой ячейки стоит **номер кадра из галереи**, а не
порядковый по листу. Это не украшение: в `framesFull` записываются именно
эти номера, и сдвиг на единицу превратил бы ссылку на кадр в ссылку на
соседний — оценку стало бы нельзя перепроверить.

Планировки помечены отдельно (жёлтым, «N · планировка»). Их в галерее
больше, чем кажется: на 244 живых лотах чертёж нашёлся у 203, и чаще всего
он **первый или второй** кадр. Лист из девяти ячеек почти в половине
случаев тратил одну на чертёж.

```bash
# все девять ячеек — квартира, планировки на лист не идут
node tools/cian/cian.js verify --ids 326035617 --photos 9 --skip-layout --dir sheets
```

Порог «мало фото» считает **кадры квартиры**, а не строчки галереи. Но там,
где Циан галерею не разметил (41 лот из 244 — поле пустое у всех кадров
разом), число комнатных кадров остаётся неизвестным, и в разборе так и
написано: «галерея не размечена». Неизвестно — не ноль.

Чего разметка не ловит: **вид из окна, дом снаружи и двор планировками не
считаются и никак не метятся**. На разобранном листе кадр №4 оказался
панорамой города. Правило «лобби, холлы, паркинг и двор — не квартира»
по-прежнему держится на глазах, а не на поле.
...
## Возраст ремонта: что различает, когда буква не различает

Буква мерит уровень материалов — и в массовом сегменте перестаёт работать.
Проверено на разборе флиппинга у Литвина-Седого: двенадцать сопоставимых
квартир 28–40 м², и **все двенадцать — C**. При этом среди них были свежий
сканди-флип, розовый ремонт 2010-х, «дорогой ремонт 2008 года» с чёрными
глянцевыми потолками и квартира с эмалированной плитой. Разный товар, одна
буква. Различает их не уровень — **возраст и износ**, и цену в сегменте
делает именно он (плюс этаж и вид).

Возраст ортогонален букве и записывается отдельно:

| Возраст | Как узнать |
|---|---|
| `свежий` | ремонт закончен, жизни не было: матрас в плёнке, пустые полки, краска без единой потёртости, санузел сухой, без налёта на смесителе |
| `жилой` | стиль актуального десятилетия, вещи по местам, износ фоновый: затёртость у ручек, магниты на холодильнике, посуда |
| `устаревший` | отделка целая, но стиль прошлых десятилетий — жить можно, продать за счёт ремонта нельзя |
| `под ремонт` | показывать нечего: мебель и плита прошлой эпохи, линолеум, ковролин, цветная плитка советских лет |

### Датировка: по чему читается десятилетие

Ремонт выдаёт себя набором решений — как одежда на старой фотографии.
Смотреть на потолок, кухонные фасады и цвет пола: они меняются модой и
почти никогда не переделываются частично.

| Эпоха | Приметы |
|---|---|
| 1995–2005 | арки из гипсокартона, многоуровневые потолки, зеркальные шкафы-купе, линолеум и ковролин, стенки ДСП, цветная плитка с бордюром |
| 2005–2015 | **глянцевый натяжной потолок**, точечные по периметру, венге + капучино, скинали с фотопечатью, керамогранит «под камень», декоративный камень на углах, ПВХ-панели |
| 2015–2020 | серый ламинат, белые глянцевые кухни, «лофт»-кирпич из гипса, серые стены, первые трек-споты, LED-ленты в нишах |
| 2020+ | ёлочка или шеврон светлого дуба, кабанчик, чёрная арматура и фурнитура, фасады без ручек, рейки, латте и греж, терраццо, подоконник-столешница |

Один элемент — не датировка: трек-споты ставят и в 2008-й ремонт при
обновлении. Датируют **три и больше примет одной эпохи**, и самая
инерционная из них — потолок: натяжной глянец не переклеивают при
косметике.

**Приметы записываются, а не подразумеваются.** В записи есть поле
`signs` — список слов из словаря `ERA_SIGNS` (таблица выше плюс приметы
дорогого сегмента: наборный мрамор со звездой, вишнёвый шпон с витражами,
патина с золотом, подсвеченный оникс, светодиодные кольца, латунные стойки,
мрамор книжным раскроем и так далее). `eraFromSigns` считает: три приметы
одной эпохи — датировано; поровну между эпохами — эпоха не выбрана;
незнакомое слово допустимо, попадает в объяснение, но в счёт не идёт.

Мебель ремонт не датирует и в приметы не записывается: она уезжает с
хозяином. Честерфилды и рояль говорят о владельце, а не об отделке.

Отдельно `wear` (следы жизни: затёртый пол у входа, потемневшая затирка,
залоснившиеся фасады) и `fresh` (матрас в плёнке, пустые полки, краска без
потёртостей). Возраст по-прежнему ставит человек, но движок возражает
(`ageCheck`): «свежий» при трёх приметах 2005–2015 — это косметика поверх
старого ремонта, «свежий» при двух следах износа — не свежий. Возражения
попадают в запись полем `ageNotes` и в оговорки оценки.

### Износ и предпродажная косметика

Износ читается по маршрутам: пол затёрт у входной двери и вдоль дивана,
выключатели желтеют, затирка темнеет у душа, фасады лоснятся у ручек.
Один-два таких следа — «жилой»; след в каждом кадре — ступень вниз.

Отдельно распознаётся **косметика под продажу** — не возраст, а грим:
свежепокрашенные стены при старом поле; новый ламинат при дверях и косяках
прошлой эпохи; пустая квартира с одним новым матрасом в плёнке; новая
фурнитура при старой сантехнике. Это сигнал считать так, будто грима нет:
покупатель заплатит за него один раз, на просмотре.

### Что менять — скелет сметы

К возрасту прилагается перечень `works`: окна, полы, стены, потолки, двери,
кухня, санузел, проводка — каждое `менять` / `освежить` / `оставить`, и
только по тому, что видно. Позиция записывается словом либо словом с
причиной: `кухня: { state: 'менять', why: 'фасады венге, накладная вытяжка' }`.
Причина — то, что видно на кадре и из чего следует «менять»; без неё
объяснение вывода приходится сочинять задним числом, и его нельзя
проверить по кадру. Из перечня выводится объём: капитальный /
частичный / косметика. Это не смета — смета считается через `finishCost` по
площади и классу, — но именно перечень отвечает на вопрос «3,5 млн хватит?»
предметно, а не интуицией.

### Судьба ремонта: въезжать или ломать

Буква говорит, **какого уровня** отделка; возраст — **какого она года**;
покупателю нужен третий ответ, и он не выводится ни из того, ни из другого
по отдельности. Дорогой ремонт 2009 года и дешёвый ремонт 2009 года ждёт
одна судьба; свежий ремонт уровня C и свежий уровня A оба позволяют жить.

`renoFate(age, worksScope)` даёт три значения:

| Значение | Когда ставится | Что это в деньгах |
|---|---|---|
| `жить как есть` | `свежий`, либо `жилой` при косметическом объёме | ремонт наследуется целиком |
| `освежить` | `жилой` с частичным объёмом; `устаревший` при косметике | наследуется половина |
| `под замену` | `под ремонт`; капитальный объём при любом возрасте; `устаревший` сверх косметики | не наследуется ничего, **и сверху демонтаж** |

Два правила стоит запомнить отдельно.

**Перечень работ старше слова.** «Свежий», в котором по кадрам меняют почти
всё, свежим не остаётся: так ловится предпродажная косметика — новая краска
поверх старой геометрии.

**Дорогой чужой ремонт хуже голой оболочки.** Оболочку не надо вывозить, а
ремонт под замену — надо, и это отдельная строка сметы. Именно поэтому
квартира с ремонтом «A, устаревший» может оказаться дороже в эксплуатации,
чем бетон по соседству.

### Цена въезда — единственный честный способ сравнить

Цена метра в объявлении сравнивает оболочку с готовой квартирой **молча и
неверно**: в одной цифре сидит товар, в который вложено ноль, и товар, в
который вложено 400 тысяч за метр.

`entryPrice(lot, fate, tier)` складывает цену и то, что придётся доделать:
полную смету для оболочки, половину при `освежить`, ноль при `жить как
есть`, а при `под замену` — полную смету плюс 12% на демонтаж и вывоз.
Только в этих деньгах два товара сравнимы.

Обе цифры — 300/400/500 тысяч за метр в классе «делюкс» и 12% на демонтаж —
**допущения**, вынесенные в код рядом с `FINISH_COST` затем, чтобы любой
вывод «дешевле на 26%» можно было пересчитать под другую смету, а не искать
её в тексте ответа.

### Оценка для читателя: без внутренней кухни

Запись оценки сделана для движка: восемь признаков, буква, улики,
перечень. Покупателю нужен ответ на три вопроса — **что за планировка, что
за ремонт и что с ним делать** — и объяснение, *почему* «въезжать» или
«ломать». `assess(record, { lot, view, house })` собирает его, а
`assessText` печатает пятью строками:

```
Итог: Покупать как оболочку и ломать.
Планировка: 4 комн. на 232 м² — по 58 м² на комнату, крупные помещения; кухня 15 м²;
  перегородки стоят: планировка задана, менять — через согласование; 2-й этаж из 8.
Ремонт: качественный полный ремонт конца 1990-х — начала 2000-х: плитка, серийные
  шкафы, кухня со встроенной техникой, свет базовый, мебель полная, паркет ёлочкой.
Почему:
  — Отделка датируется 1995–2005: вишнёвый шпон с витражами, паркетная розетка-медальон,
    хрусталь с цветными подвесками, ламбрекены — 4 приметы одной эпохи.
  — Следы жизни: затёртый пол у входа.
  — менять: стены (обои дамаск), потолки (многоуровневые ниши), кухня (патина, накладная
    вытяжка); освежить: полы (циклевать); оставить: двери. Из 5 видимых позиций менять 3,
    освежить 1 — по объёму это капитальный ремонт.
  — Когда менять приходится большую часть видимого, возраст уже не важен: под замену.
    Покупатель не наследует ничего и сверху платит за демонтаж.
Оговорки: на кадрах нет: санузлы; оригиналы кадров не открывались — оценка по контактному листу.
Дом и адрес: дом 2004 года (2000-е); 775 862 ₽/м².
```

Три правила, по которым это построено.

**Объяснение собирается, а не сочиняется.** Каждая строка «почему»
берётся из поля записи: приметы — из `signs`, износ — из `wear` и `fresh`,
перечень — из `works` с причинами, последняя строка — то правило
`renoFate`, которое сработало, пересказанное словами. Если поля нет,
объяснение так и говорит («датирующие приметы не записаны») и заносит
это в оговорки, а не молчит.

**Внутренней кухни в тексте нет.** Ни имён признаков, ни букв, ни ключей
записи: буква A становится «авторским премиумом», `stone: слэб` — «камнем
цельными плитами», `planWalls: нет` — «планом без перегородок». Всё, что
можно перепроверить по кадру, названо словами кадра. Тест на это стоит в
`test.js` и ловит утечку любого внутреннего слова.

**Планировка — только измеримое.** Метры на комнату из полей, стены по
плану из улик, этаж и его полоса. План со стенами при голом бетоне — это
дизайн-проект продавца, а не факт, и текст говорит именно так.

Команда: `assess --marks marks.json --lots lots.json [--ids] [--md] [--views вид.json]`.

### Вывод — обязателен

Запись с возрастом не принимается без вывода (`verdict`). Вывод — не
пересказ признаков, а суждение, отвечающее на три вопроса: **что это за
товар и для кого; что его продаёт или топит; что с ним сделает следующий
владелец.** Два примера с того же разбора:

> Дорогой ремонт 2008 года — чёрные глянцевые потолки, круглая кровать,
> зеркала: вложено много, вернётся ноль. Покупатель заплатит как за жилое
> состояние соседа и вычтет демонтаж.

> Образцовый бюджетный флип: сканди-косметика поверх старой геометрии,
> санузел не показан ни одним кадром — вероятно, не делался. Так выглядит
> потолок цены за отделку в этом ряду.

Правило простое: если вывод можно приложить к другой квартире той же
когорты и он не станет ложью — это не вывод, а общие слова.
...
### Пять ступеней вместо двух, и планировка как главная улика

«Жить можно или нельзя» решает, попадёт ли лот в когорту сравнения. Сколько
стоит довести — вопрос отдельный, и двоичного ответа на него мало: бетон и
whitebox различаются на стяжку и штукатурку, а «ремонт идёт» — это чужой
проект, который придётся принимать или ломать.

Повод был конкретный. **Циан на вторичке состояние отделки отдельным полем
не отдаёт вовсе.** Поле `repairType` приходит ТОЛЬКО из карточки, в выдаче
поиска его нет; на разборе Хамовников 66 квартир из 195 остались
«неизвестно», и медианы по состоянию считались на том, что удалось вычитать
из текста продавца. Текст пишет продавец, и пишет он в свою пользу.

Планировка из объявления отвечает на главный вопрос прямо и без слов:
возведены внутренние стены или нет. Поэтому на контактном листе она идёт
**первой ячейкой** — в общей куче из шестнадцати кадров её было видно
последней или не видно вовсе:

```bash
node tools/cian/cian.js verify --from lots.json --photos 12 --cols 4 --layout-first --dir sheets
```

Оценщик заполняет **улики, а не вывод**. `null` значит «не видно», и это не
то же самое, что «нет»:

| Улика | Что это |
|---|---|
| `planShown` | есть ли среди кадров планировка |
| `planWalls` | `есть` / `нет` / `частично` — возведены ли на плане внутренние перегородки |
| `roomsShown` | сколько разных помещений **самой квартиры** показано |
| `bareConcrete` | голый бетон, кирпич, стяжка без покрытия, открытая разводка |
| `wallsPlastered` | стены выровнены или окрашены, но отделка не финишная |
| `floorFinished` | уложено чистовое покрытие пола |
| `furniture` | расставлена мебель: квартира обжита или меблирована |
| `renovationInProgress` | работа **применена**: подняты каркасы, зашиты листы, уложена плитка, разведена по месту проводка, в деле инструмент |
| `render` | кадры интерьера — визуализация, а не съёмка |

**Рендер обнуляет интерьерные улики целиком.** Это не придирка, а замер: на
первом же прогоне по Хамовникам шесть лотов Фрунзенского квартала получили
от оценщиков «жилое» — на кадрах уложенный паркет, расставленная мебель,
накрытый стол. Дом сдаётся в 2027 году, продажа по ДДУ, `decoration` =
`without`: на этих кадрах нарисовано то, чего нет и два года не будет.
Планировка при рендере остаётся в силе — чертёж и есть чертёж, нарисован он
или отсканирован.

Состояние выводится из улик механическими правилами, и **план весомее
кадров**: свободная планировка — это бетон, сколько бы красивых слов ни
стояло в описании и как бы нарядно ни выглядел холл. Ровно этот случай был
у пентхауса на Льва Толстого: «стены не возведены» на плане при панорамных
окнах на кадрах.

| Состояние | Когда |
|---|---|
| `бетон` | план без перегородок, либо голый бетон и пол не уложен |
| `ремонт идёт` | работа уже скрыла бетон, но отделка не закончена |
| `whitebox` | стены выровнены, голого бетона в кадре нет, пол не уложен |
| `под ключ` | пол уложен, мебели нет |
| `жилое` | пол уложен и мебель расставлена |

**Завезённый материал — это не работы.** Правило куплено дорого: я один раз
переставил порядок так, чтобы «следы работ» били голый бетон, и переставил
на ложном основании. Пентхаус на Льва Толстого оценщик прочитал как «ремонт
идёт» по уликам «штабели газоблока, перегородки выложены наполовину,
ограждающая лента, часть стен покрашена». На исходных кадрах оказалось
обратное: голая плита, пыльная стяжка, стояки в утеплителе, оштукатуренный
застройщиком периметр — и поддоны газоблока, **сложенные штабелями** и
обнесённые лентой. Ничего не кладут; инструмента, лесов, каркасов,
протянутой проводки, начатой плитки нет ни на одном кадре.

Сложенный материал, ограждающая лента, лампочка на проводе и открытые
стояки — **обычный вид нетронутой оболочки**. Работы узнаются по
применённому: подняты каркасы, зашиты листы, уложена плитка, разведена по
месту проводка, в деле инструмент. Пока на кадрах господствует голый бетон
и пол не уложен — это бетон, сколько бы поддонов в нём ни стояло.

**План без перегородок сильнее всего остального.** Свободная планировка —
бетон, сколько бы работ на кадрах ни шло.

И общий вывод, который здесь стоит записать отдельно: **правило, выведенное
из одного лота, стоит ровно столько, сколько стоит прочтение этого лота.**
Контактный лист даёт «что в квартире есть», но «настоящее ли это» решается
только в оригинале — [два шага](#два-шага-и-второй-обязателен) не зря стоят
в начале этого файла, и менять правила движка, не пройдя второй, нельзя.

Уверенность ставится отдельно: план и три с лишним помещения — высокая,
что-то одно — средняя, интерьера нет — низкая. **При низкой уверенности
состояние по кадрам не ставится вовсе** — «под ключ» по умолчанию был бы
подарком продавцу ровно там, где он в этом больше всего заинтересован.
Расхождение с текстом остаётся в записи полем `stateProof.conflict`.

Отдельная ловушка, из-за которой правило про «кадры дома» написано в
инструкции оценщику прописными: объявление сплошь и рядом состоит из лобби,
бассейна, фасада и паркинга целиком. Тогда `roomsShown = 0`, все признаки
«не видно», и это **правильный ответ**, а не отказ от работы.
...
открывали в оригинале), `age` (возраст ремонта), `works` (что менять),
`verdict` (критический вывод — обязателен при возрасте) и `note` — что
именно видно, своими словами.
...
**Эталонный лист какое-то время стоял без примера «эконом».** Среди
тридцати одного премиального лота чистого эконома не нашлось: самое дешёвое
— рамочный МДФ на Ленинском, нижний край «встроенной». Ставить выдуманный
пример хуже, чем оставить пробел, — пробел и стоял, пока массовый сегмент
не принёс настоящий эталон: кухня на Шмитовском 13 (шкафчики прошлой эпохи,
эмалированная плита, газовая труба через фартук). Лист пересобран с ним.
