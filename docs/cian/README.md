...
`--all` объединяет выдачу по четырём сортировкам: у каждой свой обрыв.
Этого мало. На районном запросе замер дал 168 лотов обычным обходом против
**238** развёрткой с перебором сортировок — дробление и сортировки ловят
разное, и полным получается только их сочетание:
