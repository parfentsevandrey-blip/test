...
`offerCount` — оценка, а не точное число. Выдача обрывается на третьей-четвёртой
странице, и до заявленного счётчика не добирает: для ЗАО-поиска Циан заявляет
88, перечислить удаётся 80. Разбиение по ценовым диапазонам не помогает — сумма
по диапазонам даёт те же 80, а отдельные диапазоны сами отдают меньше, чем
заявляют (24 заявлено, 19 отдано на первой же странице).

Что работает — объединение по разным сортировкам: у каждой свой «хвост».
`search --all` проходит `по умолчанию`, `price_object_order`,
`creation_date_desc`, `area_order` и объединяет результат: 84 вместо 80.
Это практический потолок; расхождение с 88 — на совести счётчика Циан.
