...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      note('Камерный квартал между Чапаевским парком и Ходынским полем. Основа: Яндекс Карты.'),
      ...
      body('Площади квартир в проекте — от 28 до 161 м². Ниже — три типовых варианта, по одному на каждую основную комнатность, с параметрами конкретных лотов из текущей экспозиции.', { after: 200 }),
      ...
      body('Чтобы сравнить честно, приведём все проекты к одному состоянию — квартире, в которую можно заехать. У конкурентов для этого берём лоты, которые уже продаются с ремонтом. К цене МАСТЕРС добавляем отделку, которой в ней нет: для премиального сегмента это 150 тыс. ₽/м².', { after: 170 }),
      image('chart.jpg', PX, 303, { after: 60 }),
      caption('Расчёт по выгрузкам Циан от 28.07.2026, сопоставимая выборка 40–140 м².'),
      note('Если уложиться в 100 тыс. ₽/м², метр в МАСТЕРС выйдет в 859 654 ₽ — чуть дешевле «Династии» (867 222 ₽). При 200 тыс. — 959 654 ₽. Дороже МАСТЕРС в любом сценарии остаётся только «Прайм Парк».'),
      ...
      ...bullets([
        ...
        [txt('Если привести всё к квартире, в которую можно заехать, метр в МАСТЕРС стоит '), txt('909 654 ₽', { bold: true }), txt(' — цена застройщика плюс отделка. Дороже только «Прайм Парк» (1 045 515 ₽). «Династия» — 867 222 ₽, «Царская площадь» — 709 453 ₽, «Лица» — 604 614 ₽, «Лайнер» — 495 143 ₽.')],
        ...
        [txt('Заявленная скидка 10 % снижает полную цену метра до '), txt('833 689 ₽', { bold: true }), txt('. Это уводит МАСТЕРС ниже «Династии», но всё ещё оставляет дороже «Царской площади», «Лиц» и «Лайнера».')],
      ...
      image('map_prim.jpg', PX, 372, { after: 60 }),
      ...
      body('Обратная сторона тоже есть. Верхняя часть премиального рынка локации всё-таки дороже: в «Прайм Парке» 23 готовых лота площадью 70–100 м² стоят от 69,5 млн ₽ (962 604 ₽/м²), в «Династии» — от 67,0 млн ₽. Но МАСТЕРС с учётом будущей отделки — 73,7 млн ₽ за тот же метраж — эту нижнюю границу уже превышает.', { after: 130 }),
      body([
      ...
      ...bullets([
        ...
        'Ключевые риски: ранняя стадия строительства (готовность около 1 %) при сроке ввода IV кв. 2029 г. и отсутствие отделки во всех лотах экспозиции — метр к моменту заселения обходится примерно в 910 тыс. ₽ против 760 тыс. ₽ по прайсу.',
      ]),
