...
## Стекло 3.1: кант по всему контуру

<img src="docs/images/glass-rim.jpg" width="100%" />

В 2.5.0 «Стекло 3.0» пересчитало кромку как оптику плиты с фаской, и стекло стало хуже. Светлый
кант остался только на углу, откуда падает свет, — фаской в левом верхнем углу. На остальных
сторонах он почти погас. В 2.5.1 это откатано, и стекло сделано заново, от 2.4.1:

- **Кант читается по всему контуру.** Как у Apple, ярче всего угол, обращённый к свету, почти так
  же — противоположный, где выходит свет, прошедший сквозь стекло. По бокам кант сохраняет
  не меньше 40% света, так что у каждой панели есть край.
- **Толщина.** От кромки внутрь: тонкий блик по самому краю, светящаяся полоса отражённого неба
  (фаска чуть разлагает её в радугу) и свет, удержанный в толще стекла, прямо под ней.
- **Отсвет по телу стекла** со стороны, откуда падает свет.
- **Исправления артефактов из 3.0 остались.** Панели у края экрана не читают пустоту за сценой
  (раньше это давало голубую линию), а сложенные блики не пересвечивают цвет.
- **Переключатель режимов** по-прежнему перетекает, как капля, но оседает быстрее: примерно за
  0,7 с вместо секунды.

Кант замерен тестом (`GlassLabTest.rimReadsAllTheWayRound`) при обычном свете, без солнца на
экране. Капсула и карточка, днём и ночью, для каждой стороны: насколько кромка ярче стекла в 14 dp
от неё. Капсула днём, стороны верх / низ / лево / право:

| | верх | низ | лево | право |
|---|---|---|---|---|
| 2.4.1 | 71 | 18 | 73 | 18 |
| 3.0 (2.5.0) | 46 | 6 | 48 | 0,3 |
| 3.1 (2.5.1) | 73 | 59 | 72 | 53 |

Тест требует заметный кант с каждой стороны и не меньше 30% яркости самой светлой стороны на
самой тёмной. На стекле 3.0 он падает.
...
реальном экране новые картины, анимации недель и стекло. Урок 2.5.0: рендер Robolectric
показывал стекло 3.0 верно, но я сверял его на лабораторном стенде с низким солнцем, а не на
настоящих экранах при обычном свете. Теперь стекло сравнивается на девяти экранах приложения
(день, ночь, дождь, снег, вечер, прокрутка, настройки, города), а кант по всему контуру проверяет тест. Эффекты неба можно ограничить настройкой «Эффекты неба → Эконом».
