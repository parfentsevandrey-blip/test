...
выключается в студии). Лаунчер не даёт преломлять обои, поэтому край нарисован так, как на нём
ведёт себя свет, — и, как в приложении, этот свет даёт небо, которое виджет показывает:

<img src="docs/images/widgets-light.jpg" width="100%" />

- кромка загорается **со стороны настоящего солнца или луны** — утром слева, в полдень сверху,
  вечером справа, на закате у самого горизонта — и **их светом**: белым днём, золотым низко над
  горизонтом, серебристым от луны; насколько ярко — решают облака и фаза луны;
- где кромка смотрит на солнце, вспыхивает **блик-звёздочка**: горячее ядро, луч вдоль кромки и
  короткий поперёк, со свечением, выходящим за край;
- остальное мягко подсвечивает **небо** сверху слева; в пасмурный день и безлунной ночью светит
  только оно — без бликов, как и должно быть под облаками;
- по стеклу лежат **полосы отражённого света**, а стеклянные цифры температуры ловят тот же свет.

Код между обновлениями лаунчер не исполняет, поэтому свет пересчитывается при каждом обновлении
виджета — и ходит по нему вслед за солнцем. Волосяная кромка почти исчезает вдоль сторон, дисперсия
лёгкая: рамка вокруг всей панели в масштабе виджета читалась бы как обводка, а не как стекло.
