...
BASES = [('Веспер Кутузовский', L['Веспер Кутузовский'], 'до 19 этажей'),
         ('Бадаевский',         L['Бадаевский'],         '18 этажей'),
         ('Оба дома вместе',    LOW,                     'до 19 этажей'),
         ('Дом Дау — не брали', L['Дом Дау'],            '85–91 этаж')]
KEY = {'Оба дома вместе': 'Малоэтажные вместе', 'Дом Дау — не брали': 'Дом Дау'}

rows, out = [], {}
for name, lots, height in BASES:
    per, r2, n, lo, hi = fit(lots)
    rows.append([name, str(n), height, f'{per:+.1f} %'.replace('.', ',')])
    out[KEY.get(name, name)] = {'per': per, 'r2': r2, 'n': n, 'lo': lo, 'hi': hi}
