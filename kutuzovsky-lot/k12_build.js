...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      h2('Причина 1. В цене надбавка за этаж, которую рынок не платит'),
      ...bullets([
        [txt('В том же доме продаётся квартира-близнец. '), txt('Те же 95,0 м², тот же дизайнерский ремонт, тот же корпус — второй этаж, 133,0 млн ₽. В выгрузке эти два лота отличаются только этажом и ценой.', { bold: true })],
        'Сколько стоят четыре этажа, можно измерить. У соседей — «Веспера Кутузовского» и «Бадаевского» — 232 лота от застройщика, все без отделки, дома до 19 этажей: цена метра растёт на 2,1–3,1 % за этаж. Это чистый замер: один продавец, один прайс, никакой разницы в ремонте.',
        [txt('Четыре этажа стоят +8,7…+12,8 %. '), txt('К 133,0 млн ₽ это добавляет 11,5–17,0 млн ₽ и даёт 144,5–150,0 млн ₽. Просят 162,5 млн ₽ — то есть '), txt('12,5–18,0 млн ₽ сверх того, что рынок платит за этаж', { bold: true }), txt('.')],
      ]),
      image('floor.jpg', PX, 276, { after: 50 }),
      caption('Разница между лотами — только этаж; красная линия показывает, во сколько её оценивает продавец.'),

      dataTable(
        ['База замера', 'Что это', 'Лотов', 'Этажи', 'За этаж', 'R²'],
        K.floorRows, [1900, 3560, 780, 900, 1300, 1198],
        { boldFirstCol: true },
      ),
      spacer(30),
      note('Регрессия ln(цена за м²) по этажу с поправкой на площадь. Для замера годятся только малоэтажные проекты: в 85-этажной башне прирост на этаж структурно другой — 0,15 % у «Дома Дау» против 2,3 % у соседей по Кутузовскому проспекту. Этаж объясняет треть разброса цены метра (R² = 0,33–0,40); остальное — планировка, вид и сторона света, но по этим параметрам оба лота в выгрузке описаны одинаково.'),
