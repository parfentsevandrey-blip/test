...
- «no transport could be started». Две причины. Реестр транспортов в
  lyrebird отказывает в повторной регистрации, а приложение создавало
  контроллер заново при каждом холодном старте, так что любая повторная
  попытка подключения была обречена. И групповой контейнер: macOS 15+ не
  даёт неподписанному приложению трогать `~/Library/Group Containers`, а
  состояние транспортов лежало именно там. Теперь реестр инициализируется
  один раз, а состояние живёт в Application Support, если подпись не даёт
  права на группу. Причина отказа теперь печатается прямо в окне ошибки.
