...
--loc / --topic задают свои регекспы вместо зашитых — для поиска по одной площадке.
...
if opt('loc'): LOC = re.compile(opt('loc'), re.I)      # точечный поиск по одной площадке
if opt('topic'): TOPIC = re.compile(opt('topic'), re.I)
