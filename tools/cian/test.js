...
const { normalize, groupSameFlat, dedupe, findTwins, withMarket, median, assessRepair, mergeArchive, archiveStat,
        ...
        renoFate, entryPrice, suspectTwins, RENO_FATES, eraFromSigns, ageCheck, workState, workWhy, assess, assessText,
...
test('завезённый материал — это не работы: голый бетон остаётся бетоном', () => {
  // Улики целевого пентхауса на Льва Толстого, как их прочитал оценщик:
  // «штабели газоблока, перегородки выложены наполовину, лента, часть стен
  // покрашена». На исходных кадрах — сданная оболочка, в которой ШТАБЕЛЯМИ
  // лежит завезённый газоблок; инструмента, каркасов и начатой отделки нет.
  ...
  assert.strictEqual(stateFromEvidence(e), 'бетон',
    'пока на кадрах господствует голый бетон, поддоны в углу состояния не меняют');
  // «ремонт идёт» остаётся для случая, когда работа уже скрыла бетон
  assert.strictEqual(stateFromEvidence({ ...e, bareConcrete: false }), 'ремонт идёт');
  // whitebox — работы закончены до стадии «под чистовую», бетона в кадре нет
  assert.strictEqual(stateFromEvidence({ ...e, bareConcrete: false,
    renovationInProgress: false }), 'whitebox');
});
...
test('датировка по приметам: три одной эпохи датируют, одна — нет, поровну — эпоха не выбрана', () => {
  const e = eraFromSigns(['глянцевый натяжной потолок', 'венге', 'скинали с фотопечатью', 'трек-споты']);
  assert.strictEqual(e.era, '2005–2015');
  assert.strictEqual(e.n, 3);
  assert.strictEqual(e.sure, true, 'три приметы одной эпохи — датировано, трек-споты не мешают');
  const one = eraFromSigns(['трек-споты']);
  assert.strictEqual(one.sure, false, 'одна примета — не датировка');
  assert.strictEqual(one.era, '2015–2020', 'но эпоха названа, чтобы было с чем спорить');
  const tie = eraFromSigns(['венге', 'капучино', 'шеврон', 'кабанчик']);
  assert.strictEqual(tie.era, null, 'поровну между эпохами — эпоха не выбрана, а не первая попавшаяся');
  const unk = eraFromSigns(['розовые стены', 'венге']);
  assert.deepStrictEqual(unk.unknown, ['розовые стены'], 'незнакомая примета не ошибка, но в счёт не идёт');
  assert.strictEqual(unk.n, 1);
  assert.deepStrictEqual(eraFromSigns(null), { era: null, n: 0, sure: false, by: {}, unknown: [] });
});

test('движок возражает возрасту, если приметы говорят другое', () => {
  const old3 = ['глянцевый натяжной потолок', 'венге', 'скинали с фотопечатью'];
  assert.ok(ageCheck('свежий', old3, [], []).length, '«свежий» при трёх приметах 2005–2015 — косметика поверх старого');
  assert.ok(ageCheck('жилой', old3, [], []).length);
  assert.strictEqual(ageCheck('устаревший', old3, [], []).length, 0, 'а «устаревший» с ними согласуется');
  assert.ok(ageCheck('устаревший', ['шеврон', 'кабанчик', 'чёрная арматура'], [], []).length, '«устаревший» при 2020+ — описка');
  assert.ok(ageCheck('свежий', [], ['затёртый пол у входа', 'потемневшая затирка'], []).length, 'свежий с двумя следами износа');
  assert.strictEqual(ageCheck('свежий', [], ['затёртый пол у входа'], []).length, 0, 'один след — фон, не возражение');
  assert.strictEqual(ageCheck(null, old3, [], []).length, 0, 'без возраста возражать нечему');
});

test('перечень работ принимает слово и слово с причиной, объём считается одинаково', () => {
  const plain = { потолки: 'менять', кухня: 'менять', полы: 'освежить', двери: 'оставить' };
  const rich = { потолки: { state: 'менять', why: 'натяжной глянец' }, кухня: { state: 'менять', why: 'венге' },
    полы: { state: 'освежить', why: 'циклевать' }, двери: 'оставить' };
  assert.strictEqual(worksScope(plain), worksScope(rich));
  assert.strictEqual(workState(rich.потолки), 'менять');
  assert.strictEqual(workWhy(rich.потолки), 'натяжной глянец');
  assert.strictEqual(workWhy(plain.потолки), null);
  const lot = { id: 5, totalArea: 100, priceRub: 10e6, photos: [] };
  assert.throws(() => gradeRecord(lot, { works: { кухня: { state: 'снести', why: 'x' } } }), /не из списка/);
  assert.throws(() => gradeRecord(lot, { signs: 'венге, капучино' }), /список строк/, 'приметы строкой через запятую — ошибка, а не молчаливый ноль');
});

test('оценка для читателя: почему «под замену» собирается из записанного', () => {
  const lot = { id: 7, rooms: 4, totalArea: 232, kitchenArea: 15, floor: 2, floors: 8, buildYear: 2004,
    priceRub: 180e6, photos: [], street: 'Неопалимовский', house: '8' };
  const rec = gradeRecord(lot, {
    proof: 'фото',
    markers: { stone: 'керамогранит', joinery: 'серийная', kitchen: 'встроенная', light: 'базовый',
      furniture: 'полный', bath: null, floor: 'массив ёлочкой', doors: 'в наличнике' },
    evidence: { planShown: true, planWalls: 'есть', roomsShown: 5, bareConcrete: false, wallsPlastered: true,
      floorFinished: true, furniture: true, renovationInProgress: false, render: false },
    age: 'устаревший',
    signs: ['вишнёвый шпон с витражами', 'паркетная розетка-медальон', 'хрусталь с цветными подвесками', 'ламбрекены'],
    wear: ['затёртый пол у входа'],
    works: { стены: { state: 'менять', why: 'обои дамаск' }, потолки: { state: 'менять', why: 'многоуровневые ниши' },
      кухня: { state: 'менять', why: 'патина, накладная вытяжка' }, полы: { state: 'освежить', why: 'циклевать' }, двери: 'оставить' },
    verdict: 'Дворцовая классика конца двухтысячных, сделана дорого и под один вкус; паркет и двери переживут смену хозяина.',
  });
  assert.strictEqual(rec.renoFate, 'под замену');
  assert.strictEqual(rec.era, '1995–2005');
  const a = assess(rec, { lot });
  assert.strictEqual(a.headline, 'под замену');
  assert.strictEqual(a.bottom, 'Покупать как оболочку и ломать');
  const pts = a.why.map((w) => w.point);
  assert.deepStrictEqual(pts, ['датировка', 'износ', 'перечень', 'итог'], 'цепочка: чем датировано → износ → что менять → правило');
  const text = assessText(a);
  assert.ok(/вишнёвый шпон с витражами/.test(text), 'приметы названы поимённо');
  assert.ok(/4 приметы одной эпохи/.test(text), 'и посчитаны, со склонением');
  assert.ok(/кухня \(патина, накладная вытяжка\)/.test(text), 'причина «менять» стоит рядом с позицией');
  assert.ok(/менять 3, освежить 1/.test(text));
  assert.ok(/капитальный ремонт/.test(text));
  assert.ok(/платит за демонтаж/.test(text), 'следствие для денег названо');
  assert.ok(/на кадрах нет: санузлы/.test(text), 'чего не видно — сказано');
  assert.ok(/по 58 м² на комнату/.test(text), 'планировка — из полей, а не из впечатления');
  assert.ok(/2-й этаж из 8/.test(text));
  /* внутренней кухни в тексте нет */
  for (const bad of ['markers', 'worksScope', 'planWalls', 'evidence', 'renoFate', 'stone', 'joinery',
    'null', 'undefined', '{', '}', 'level', ' B ', ' A ']) {
    assert.ok(!text.includes(bad), `в тексте для читателя не место «${bad}»`);
  }
});

test('оценка для читателя: «въезжать» с приметами свежести, и честные пробелы', () => {
  const lot = { id: 8, rooms: 5, totalArea: 224.3, floor: 3, floors: 6, buildYear: 2000, priceRub: 440e6, photos: [] };
  const rec = gradeRecord(lot, {
    proof: 'фото', framesFull: [3, 4],
    markers: { stone: 'слэб', joinery: 'на заказ', kitchen: 'интегрированная', light: 'сценарный',
      furniture: 'полный', bath: null, floor: 'массив ёлочкой', doors: 'скрытые' },
    age: 'свежий', signs: ['ёлочка светлого дуба', 'магнитные треки', 'мраморный остров с водопадом', 'скрытые двери'],
    fresh: ['постановочные цветы', 'краска без потёртостей'],
    works: { стены: 'оставить', потолки: 'оставить', кухня: 'оставить', полы: 'оставить', двери: 'оставить' },
    verdict: 'Эталон готового: въезжать с чемоданом и лет семь ничего не трогать; ремонт последних лет, ничего чужого.',
  });
  const a = assess(rec, { lot });
  assert.strictEqual(a.bottom, 'Въезжать и жить');
  const text = assessText(a);
  assert.ok(/датируется 2020\+/.test(text));
  assert.ok(/Жизни не было: постановочные цветы/.test(text));
  assert.ok(/наследуется целиком/.test(text));
  assert.ok(!/оригиналы кадров не открывались/.test(text), 'второй шаг сделан — оговорки нет');
  assert.ok(/на кадрах нет: санузлы/.test(text));
  /* без возраста судьбы нет, и это сказано, а не спрятано */
  const noAge = assess(gradeRecord(lot, { proof: 'фото', markers: rec.markers }), { lot });
  assert.strictEqual(noAge.headline, 'возраст не поставлен');
  assert.ok(noAge.caveats.some((c) => /возраст ремонта не записан/.test(c)));
});

test('оценка для читателя: оболочка, и план со стенами при голом бетоне — проект, а не факт', () => {
  const lot = { id: 9, rooms: 5, totalArea: 266, floor: 7, floors: 7, buildYear: 2014, priceRub: 550e6, photos: [] };
  const rec = gradeRecord(lot, {
    proof: 'фото',
    markers: { kitchen: 'нет', furniture: 'нет', bath: 'не отделан', light: 'нет', floor: 'стяжка', joinery: 'нет' },
    evidence: { planShown: true, planWalls: 'есть', roomsShown: 3, bareConcrete: true, wallsPlastered: true,
      floorFinished: false, furniture: false, renovationInProgress: false, render: false },
  });
  const a = assess(rec, { lot, view: { klass: 'открытый', river: 850, green: 120, aboveRoof: 2 } });
  assert.strictEqual(a.headline, 'бетон');
  const text = assessText(a);
  assert.ok(/ничего чужого ломать не надо/.test(text));
  assert.ok(/перегородки есть только на плане — это проект/.test(text), 'стены нарисованы, а не построены');
  assert.ok(/дизайн-проект продавца/.test(text));
  assert.ok(/7-й этаж из 7 — последний/.test(text));
  assert.ok(/зелень в 120 м/.test(text) && /выше линии крыш/.test(text));
  assert.ok(!/на кадрах нет:/.test(text), 'у оболочки не спрашивают про кухню и санузлы');
});
