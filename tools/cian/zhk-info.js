...
function parse(text) {
  ...
  const trim = (s) => (s || '').replace(/^Сдач[аи]\s+в?о?\s*/i, '').split(',')[0].trim();
  let delivery = trim(grab(/(?:Срок сдачи|Сдача)\s*\n\s*([^\n]{2,32})\s*\n\s*Класс/)) || null;
  ...
  const lines = t.split('\n').map((s) => s.trim());
  const iAddr = lines.findIndex((s) => /^Москва,/.test(s));
  /* Между плашкой и адресом бывают «Перейти на сайт ЖК», метка поля «Сдача» и пустые
     строки, поэтому смотрим окно до адреса и берём ближайшую содержательную плашку. */
