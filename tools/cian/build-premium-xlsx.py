...
# «Донской» — вне основного контура: зона заведена под площадку на Шаболовке,
# добавленную по отдельной просьбе. По координатам сюда ничего не попадает.
ZONE_ORDER = {'Садовое кольцо': 0, 'Хамовники': 1, 'Сити': 2, 'Пресня': 3, 'Белорусская': 4, 'Донской': 5}
...
sheet(wb, '1. Построено (вторичка)', H1, rows1,
      ...
      subtitle=f'Премиум-ЖК Садового кольца, Хамовников, Пресни и Сити со сроком сдачи 2022 года и позже. Цены — по предложениям на продажу, {MONTH}', heat_col=9)
...
sheet(wb, '2. Строится', H2, rows2,
      ...
      subtitle=f'Строящиеся премиум-ЖК Садового кольца, Хамовников, Пресни и Сити. Цены — по предложениям на продажу, {MONTH}', heat_col=9)

# ---------- лист 3 ----------
rows3 = cluster(rows3, 3, 11, ('площадка', 'площадки', 'площадок'))
# «Старт (план)», «Сдача (план)» и «Цена от» у площадок почти везде неизвестны —
# столбцы убраны; порядок внутри кластера по-прежнему считается по цене.
W3 = [24, 20, 26, 14, 18, 20, 9, 6, 9, 10, 10, 10, 10, 10, 10, 8, 8, 8, 72]
KEEP3 = [i for i in range(len(H3)) if i not in (9, 10, 11)]
H3 = [H3[i] for i in KEEP3]; W3 = [W3[i] for i in KEEP3]
rows3 = [r if r and r[0] == '__GROUP__' else [r[i] for i in KEEP3] for r in rows3]
sheet(wb, '3. Проектирование', H3, rows3, W3,
      subtitle='Площадки и анонсированные проекты без начала строительства: участки, КРТ, ГПЗУ, АГР, сделки с землёй. Июнь 2025 — сентябрь 2026, со ссылками на публикации')
...
if chs:
    ...
    sheet(wb, 'Источники — TG-каналы', HS, rs, [34, 26, 8, 70, 13, 12],
          subtitle='Отраслевые Telegram-каналы о недвижимости: публикации о площадках и новых проектах в рассматриваемых локациях, июнь 2025 — сентябрь 2026', zone_col=None)
