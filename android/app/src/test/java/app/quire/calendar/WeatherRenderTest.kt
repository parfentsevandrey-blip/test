...
class WeatherRenderTest {
    ...
    fun `the card reads at every placement it allows`() {
        val forecast = stub()
        app.quire.weather.WeatherStore.save(context, forecast)
        ...
        listOf(
            ...
            host.findViewById<android.view.View>(app.quire.calendar.R.id.strip)
    ...
    fun `a card with nothing fetched yet says so`() {
        app.quire.weather.WeatherStore.clear(context)
        val views = app.quire.weather.WeatherWidgetRenderer.build(context, 63, 340, 160)
        val host = FrameLayout(context).apply { setBackgroundColor(0xFF101014.toInt()) }
        host.addView(
            views.apply(context, host),
            FrameLayout.LayoutParams(
    ...
    private fun stub(): app.quire.weather.Forecast {
        val today = java.time.LocalDate.now()
        val skies = listOf(
            app.quire.weather.Sky.SHOWERS,
            app.quire.weather.Sky.PARTLY_CLOUDY,
            app.quire.weather.Sky.CLEAR,
            app.quire.weather.Sky.THUNDER,
            app.quire.weather.Sky.SNOW,
        )
        return app.quire.weather.Forecast(
            place = "Западный Порту",
            latitude = 55.75,
            longitude = 37.62,
            now = app.quire.weather.Conditions(
                temperature = 12.4,
                feelsLike = 10.6,
                sky = app.quire.weather.Sky.SHOWERS,
                day = false,
                humidity = 82,
                wind = 14.0,
            ),
            days = skies.mapIndexed { index, sky ->
                app.quire.weather.DayForecast(
