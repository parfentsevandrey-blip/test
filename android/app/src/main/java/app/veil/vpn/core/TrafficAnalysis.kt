...
object TrafficAnalysis {
    ...
    suspend fun throughput(socks: SocksProxy): Throughput = withTimeoutOrNull(90_000) {
        var socket: Socket? = null
        try {
            val started = System.currentTimeMillis()
            val raw = Socks5.connect(socks, SPEED_HOST, 443, CONNECT_TIMEOUT, READ_TIMEOUT)
            socket = raw
            val tls = (SSLSocketFactory.getDefault() as SSLSocketFactory)
                .createSocket(raw, SPEED_HOST, 443, true) as SSLSocket
            tls.startHandshake()
            tls.getOutputStream().apply {
                write(
                    ("GET $SPEED_PATH HTTP/1.1\r\nHost: $SPEED_HOST\r\nUser-Agent: $AGENT\r\n" +
                        "Accept: */*\r\nConnection: close\r\n\r\n").toByteArray(),
                )
                flush()
            }
