...
только Snowflake** (в России — набор Tor Project для "ru", иначе встроенные строки Tor Browser;
без гонки и без запроса к Settings API до подключения).
...
4. **SOCKS Tor — `127.0.0.1:<свободный порт>`**, выбирается перед каждым запуском Tor и не
   меняется до его остановки (не `auto` — см. п. 34), HTTPTunnelPort/ControlPort не открываются. Порты PT — случайные (IPtProxy слушает `127.0.0.1:0`).
...
32. **Snowflake для России — набор Tor Project для "ru"** (`assets/snowflake_regional.json`,
    ...
    Набор из кеша API берётся, только если ответ получен для текущей страны: без страны это
    `/defaults` (сервер не определил страну) со встроенными строками — 1.0.0 кешировала его как
    есть, и при обновлении он заменял бы набор "ru".
...
ADR 34–38 — ошибки, найденные прогоном APK в эмуляторе Android 17 (см. docs/test-report.md);
каждая по отдельности ломала подключение на телефоне.

34. **SOCKS-порт Tor фиксированный, не `auto`** (`LoopbackPorts.free()` на каждый запуск Tor).
    `DisableNetwork 1` закрывает все слушатели, кроме управляющего, а `0` открывает заново — с
    `auto` на новом случайном порту, и hev продолжал слать трафик на старый: после любой смены сети
    или переключения DisableNetwork трафик шёл в никуда при статусе «подключено». Старт без сети
    (Tor с `DisableNetwork 1`) падал с «Tor opened no SOCKS listener» → «Tor не запустился».
    Теперь движок берёт порт из конфигурации, пока сеть выключена (`CTorEngine.socksPortOf`), а
    после каждого `DisableNetwork 0` сессия проверяет `net/listeners/socks` и при отсутствии
    слушателя (порт заняли) перезапускает Tor на новом порту. Проверено в эмуляторе: после
    переключения слушатель вернулся на тот же порт.
35. **`NetworkMonitor` не считает VPN подлежащей сетью и игнорирует `onLost` чужих сетей.**
    Пока наш VPN поднимается, Android на мгновение отдаёт его колбэку сети по умолчанию нашего же
    процесса (исключённого из VPN); у VPN в capabilities есть и CELLULAR/WIFI подлежащей сети, так
    что он выглядел как «Network changed (Cellular)» → `toggleNetwork()` → обрыв первой попытки
    подключения при каждом включении (в 1.0.0 вместе с п. 34 — мёртвый трафик сразу после
    подключения).
36. **Адрес TUN IPv4 — частный `10.111.222.1`** (был 198.18.0.1). Клиент Snowflake в нашем
    процессе собирает WebRTC host-кандидаты со всех нечастных адресов интерфейсов
    (`util.IsLocal`): 198.18.0.1 уходил волонтёрским прокси в SDP (одинаковый у всех
    пользователей — отпечаток) и использовался как источник ICE-проверок. MapDNS (100.64.0.0/10) и
    DNS 198.18.0.2 не менялись.
37. **Политика watchdog — по активным мостам, а не только по режиму**
    (`ModePolicy.of(mode, configured)`): строки Snowflake не разрываются никогда — ни свои строки
    в «Свои мосты», ни Snowflake, выигравший гонку в «Авто».
38. **Запуск Tor: `SETEVENTS`/`GETINFO` ждут до 120 с** (остальные команды — 30 с). Сразу после
    старта Tor грузит кеш консенсуса и дескрипторов (на телефоне секунды, на медленном устройстве —
    минуты); закрытие управляющего соединения по таймауту убивало живой Tor, а следующий запуск
    грузил тот же кеш — петля.
39. **Уведомление — постоянное, со скоростью и кнопками «Переподключить» / «Отключить»** (по
    просьбе пользователя; без сети — только «Отключить»). «Переподключить» — новая сессия Tor
    (`TunnelController.reconnect`: новые транспорты, новый прокси Snowflake, новые цепочки) при
    поднятом VPN-интерфейсе, чтобы трафик не шёл мимо Tor; в машине состояний —
    `ConnectionEvent.Reconnect` → `Reconnecting(User)`. Скорость обновляется раз в секунду, пока
    экран включён (при выключенном её никто не видит). Android 14+ позволяет смахнуть уведомление
    foreground-сервиса — оно возвращается (`deleteIntent`). «Новая личность» из уведомления убрана:
    три длинные русские кнопки не помещаются в ряд; она на главном экране. Разрешение на уведомления
    (Android 13+) спрашивается ещё и при нажатии «Подключить», если в онбординге его не дали.
40. **Плитка быстрых настроек предлагается сама**: после первого подключения (приложение на
    экране, Android 13+) — системный диалог `requestAddTileService`, один раз
    (`AppSettings.tileOffered`); ниже Android 13 — подсказка в настройках. Выключить VPN плиткой на
    заблокированном экране с защищённой блокировкой можно только после разблокировки
    (`unlockAndRun`): иначе защиту отключит любой, у кого в руках телефон.
41. **Таб-бар — линза из стекла, которую можно тащить пальцем** (как в iOS 26). Жест — у всей
    панели: нажатие в любом месте поднимает линзу под палец, она следует за пальцем (критически
    задемпфированная пружина), растягивается по скорости, при пересечении вкладки — лёгкий тик;
    вкладка выбирается при отпускании. Рецепт — каталог Kyant0/AndroidLiquidGlass
    (`LiquidBottomTabs`, Apache-2.0): невидимая копия панели, где все вкладки нарисованы выбранными
    (акцент), записывается в `LayerBackdrop`; линза рисует `combined(floating, копия)` с
    `lens`-преломлением, хроматической аберрацией, бликом и тенями, пропорциональными подъёму, и
    увеличивает вкладки под собой в 1,2 раза. Касания панель забирает в `PointerEventPass.Initial`,
    поэтому `selectable` у вкладок срабатывает только от TalkBack и клавиатуры — двойного выбора нет.
    Без живого стекла (упрощённая графика, Android < 13) — плоская «таблетка» с тем же жестом.
    Поверхностей стекла у панели три (панель, копия, линза) — с шаром на главном экране ровно 4
    (п. 21), но копия и линза существуют только пока линза поднята или едет: в покое та же картинка
    рисуется плоской «вуалью» и акцентной вкладкой — без трёх слоёв стекла на каждый кадр авроры.
42. **Таймаут команды управления Tor — сбой, а не отмена.** `withTimeout` бросает
    `TimeoutCancellationException`, это `CancellationException`, а все вызывающие пробрасывают
    отмену дальше: зависший Tor молча завершал корутину — запуск сессии висел на «Подключение… 0 %»
    навсегда (найдено в эмуляторе Android 17: под нагрузкой Tor не ответил на `SETEVENTS` за 120 с,
    сессия тихо умерла, Tor закрылся). Теперь `awaitReply` превращает таймаут в
    `TorControlException` (так же — ожидание управляющего сокета при старте и запрос к Settings API,
    `MoatException`): старт заканчивается ошибкой «Tor не запустился», циклы (watchdog, гонка)
    обрабатывают сбой как сбой. Ожидающий остаётся в очереди — поздний ответ достаётся своей
    команде, порядок ответов не сбивается.
...
- Реального устройства в среде сборки нет. 1.0.1 прогнана в эмуляторе Android 17 (x86_64,
  программная эмуляция): VPN, MapDNS, Tor, клиент Snowflake — до настоящей сети Tor, но через
  локальные брокер и прокси Snowflake (в среде нет UDP), см. `docs/test-report.md`. Волонтёрские
  прокси Snowflake, сети в России, смена сетей, IPv6 — не проверены; чек-лист — README →
  «Проверка на устройстве». Всё, что не проверено, так и помечено в отчётах.
...
- [x] Фаза 1 — сетевое ядро: VpnService + hev + Tor + Snowflake (проверено в эмуляторе Android 17,
      на телефоне — нет).
...
- [x] 1.0.1 — Snowflake по умолчанию без вмешательства приложения, набор Snowflake для России,
      прогон в эмуляторе Android 17 и исправление найденных им ошибок (ADR 30–38).
- [x] 1.0.2 — кеш Settings API не для текущей страны не заменяет набор Snowflake для России
      (ADR 32); в эмуляторе не прогонялась, покрыто юнит-тестом.
