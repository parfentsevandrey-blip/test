...
(C-tor) и по умолчанию подключается через мосты: Snowflake, гонка транспортов (WebTunnel, obfs4,
meek) и мосты из Circumvention Settings API Tor Project. Своих серверов у приложения нет —
только инфраструктура Tor Project. Интерфейс — «жидкое стекло» (Liquid Glass) на Jetpack Compose.

> **Статус.** Приложение собирается, проходит юнит-, скриншот- и UI-тесты, lint и detekt.
> **На реальном устройстве оно не проверялось** — в среде сборки не было ни устройства, ни
> эмулятора. Всё, что требует устройства (подключение к Tor, утечки, батарея, скорость), собрано
> в чек-лист ниже. Подробности — в [отчёте о проверке](docs/test-report.md).
...
- Мосты из коробки: Snowflake, гонка транспортов в режиме «Авто», мосты из Circumvention
  Settings API (запрос маскируется под обращение к CDN — как в Tor Browser), свои мосты
  (вставка, QR-код с камеры или из картинки).
...
- Watchdog против «заморозок» соединения: новые цепочки → другой мост/транспорт → перезапуск Tor.
- Честное состояние «Сеть блокирует все способы подключения» с подсказками, что делать.
...
- Чтобы подобрать мосты для вашей сети, приложение один раз обращается к серверу Tor Project
  (Circumvention Settings API) **напрямую, не через Tor**: сервер видит ваш IP-адрес, как и в
  Tor Browser. Об этом говорится при первом запуске.
...
Если «Авто» не справляется (сеть с «белыми списками», агрессивная блокировка):
...
10. Гонка: сломать Snowflake (debug-сборка:
    `adb shell run-as app.opal.debug touch files/debug_break_snowflake`, переподключиться) —
    приложение само побеждает через другой транспорт; в «Подключении» видно, что сработало.
