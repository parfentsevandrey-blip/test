...
> списка выходных узлов Tor; в 1.0.3 — уведомление (скорость, «Переподключить», «Отключить»,
> возврат после смахивания), плитка быстрых настроек и перетаскивание стеклянной линзы по нижней
> панели. Волонтёрские прокси Snowflake отсюда недоступны (в среде сборки нет
...
- Постоянное уведомление: скорость ↓/↑ (обновляется каждую секунду, пока включён экран), кнопки
  «Переподключить» и «Отключить»; смахнутое — возвращается, пока VPN включён.
- Плитка в быстрых настройках (после первого подключения приложение само предлагает её добавить;
  выключение с заблокированного экрана — только после разблокировки), «Постоянная VPN».
- Нижняя панель — стеклянная линза: зажмите любую вкладку и ведите палец, не отрывая, — линза
  едет за пальцем и увеличивает вкладки под собой; вкладка выбирается при отпускании.
