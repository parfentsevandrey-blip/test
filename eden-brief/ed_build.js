...
const SRC = [
  ...
  ['Цены конкурентов — выгрузки Циан от 17.08.2026 по шести проектам: «Фамильный дом Люче» 26 лотов, «Кло 17» 17, «Никитский-6» 32 объявления на 16 лотов, Turandot Residences 8, «Брюсов» 4, Stella di Mosca 3. Всего 74 квартиры после схлопывания дублей', ''],
  ['Параметры и срок сдачи клубного дома «Брюсов» (Vos’hod, Брюсов пер., 2/14с10) — карточки проекта на «Новострой.ру» и «Элитное.ру»', 'https://www.novostroy.ru/buildings/bryusov-by-loro-piana/'],
  ['Визуализации проекта, хроника согласований и отраслевые оценки цены — публикации в Telegram-каналах «Старты продаж» (посты 9568, 9725, 11406) и Property Insider (пост 32429), апрель 2024 — декабрь 2025', 'https://t.me/startyprodazh/11406'],
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      factSheet(K.facts),
      spacer(40),
      note('Сам EDEN продаётся закрыто: публичной рекламной кампании нет, прайс-лист застройщик не публикует, объявлений на площадках нет. Единственная опубликованная ценовая цифра — старт продаж «от 1,1 млрд ₽ за резиденцию площадью около 250 кв. м», и всё, что посчитано из неё, помечено словом «расчёт». Зато по шести соседним клубным домам данные полные: 74 квартиры из выгрузок Циан от 17.08.2026.'),
      spacer(40),
      kicker('Расположение и ближайшие конкуренты', INK),
      image('map.jpg', PX, 391, { after: 46 }),
      note('Все шесть конкурентов стоят в пределах километра от EDEN, а четыре из них — ближе полукилометра. Это один и тот же кусок города вокруг «Арбатской», Воздвиженки и Большой Никитской. Цена метра в подписях — из выгрузок Циан от 17.08.2026, у EDEN — расчёт по старту продаж. «Stella di Mosca» (Б. Никитская, 9/15) в таблицах участвует, но на карту не нанесена: координаты дома подтвердить не удалось.'),
      ...
      body('Прайс-листа у EDEN нет, поэтому отталкиваемся от единственной опубликованной цифры: старт продаж «от 1,1 млрд ₽ за резиденцию площадью около 250 кв. м». Делим одно на другое — получаем 4,4 млн ₽ за метр. А вот по конкурентам данные полные: шесть клубных домов в пешей доступности, 74 квартиры после схлопывания дублей.', { after: 180 }),
      dataTable(
      ...
      spacer(40),
      ...bullets([
        [txt('EDEN дороже 68 квартир из 74. '), txt('Средняя по конкурентам — 2 973 тыс. ₽ за метр, у EDEN расчётные 4 400 тыс.: это на 48 % выше. Более дорогой метр только у шести лотов, и половина из них — в одном доме.', { bold: true })],
        [txt('Этот дом — «Брюсов», и он единственный, кто стоит дороже EDEN. '), txt('Клубный дом Vos’hod в Брюсовом переулке, в полукилометре от EDEN. В продаже четыре резиденции 208,7–329,3 м² по 1,00–1,81 млрд ₽ — метр от 4 318 до 6 843 тыс. ₽, в среднем 5 608 тыс. Это выше расчётного метра EDEN «под ключ» (5 150 тыс.) на 9 %. То есть потолок цены в этой локации держит не EDEN, и потолок этот доказан живыми объявлениями.', { bold: true })],
        [txt('Но «Брюсов» сдаётся в 2026 году, а EDEN — в 2029-м. '), txt('И квартиры в «Брюсове» идут с готовой отделкой по дизайну Loro Piana Interiors, а не с бетоном. Получается, что за котлован со сдачей через три года просят почти столько же, сколько за готовый дом с ремонтом: разница в метре — около 8 %. Это и есть вся премия покупателя за три года ожидания и за строительный риск.', { bold: true })],
        [txt('Нижнюю границу задаёт «Кло 17» на Староваганьковском. '), txt('Тот же формат клубного дома в центре, те же 17 квартир без отделки от застройщика — самый близкий аналог по продукту. Метр там 2 627 тыс. ₽, то есть EDEN дороже на 67 %. С учётом отделки у обоих: 5 150 против 3 377 тыс. ₽ — разрыв 52 %.', { bold: true })],
        [txt('Итого EDEN попадает в верхнюю часть диапазона, но не выше всех. '), txt('Разброс метра по подборке — от 830 тыс. до 6 843 тыс. ₽. EDEN с расчётными 4 400 тыс. стоит между «Кло 17» и «Брюсовым», ближе к верхней границе.')],
      ...
      spacer(40),
      ...bullets([
        [txt('Дороже миллиарда рублей во всей подборке — шесть лотов из семидесяти четырёх, и четыре из них в «Брюсове». '), txt('Плюс 358,9 м² в «Фамильном доме Люче» за 2,15 млрд ₽ и 198,1 м² в «Stella di Mosca» за 1,07 млрд ₽. В EDEN же дороже миллиарда по расчёту большинство из 22 резиденций — то есть один дом добавит к этому сегменту втрое больше квартир, чем в нём сейчас стоит целиком по всей округе.', { bold: true })],
        ...
        [txt('И отдельно — про саму цифру 4,4 млн ₽ за метр. '), txt('Она получена из формулировки «от 1,1 млрд ₽», то есть это цена самого дешёвого лота, а не средняя по дому. Средний метр по EDEN почти наверняка выше расчётного, и тогда разрыв с «Брюсовым» сокращается или исчезает вовсе. Пока прайса нет, точнее сказать нельзя — но ошибка здесь смещена в одну сторону: реальные цены будут не ниже расчётных.', { bold: true })],
      ]),
      ...
      ...bullets([
        ...
        [txt('Ожидания по цене в отрасли изначально были высокими. '), txt('В декабре 2025 года в отраслевом канале прогнозировали, что Sense выйдет с ценой заметно выше рынка, а «Никитский-6» оценивали «на уровне 3 миллионов с метра». Обе оценки подтвердились: по нашим выгрузкам «Никитский-6» идёт по 3,19 млн ₽ за метр, а расчётная цена EDEN — 4,4 млн.', { bold: true })],
      ]),
      ...
      ...bullets([
        ...
        [txt('Цена — в верхней части рынка, но не выше всех. '), txt('Расчётные 4,4 млн ₽ за метр дороже 68 квартир из 74 в шести соседних клубных домах. Нижнюю границу задаёт «Кло 17» без отделки — 2,63 млн ₽ за метр, EDEN дороже на 67 %. Верхнюю держит «Брюсов» — 5,61 млн ₽ за метр «под ключ» против расчётных 5,15 млн у EDEN.', { bold: true })],
        [txt('И вот это — главный вопрос к цене. '), txt('«Брюсов» в полукилометре сдаётся в 2026 году и продаётся с готовой отделкой, EDEN — котлован со сдачей в 2029-м без отделки. Разница в метре «под ключ» между ними всего около 8 %. То есть за три года ожидания и строительный риск покупатель получает скидку меньше десятой части цены. Причём 4,4 млн — это цена «от», по самому дешёвому лоту; средний метр по дому будет выше, и эта разница может исчезнуть совсем.', { bold: true })],
        [txt('Главный риск — ёмкость спроса. '), txt('Дому нужно продавать около семи резиденций в год дороже миллиарда рублей. Весь московский рынок таких сделок — 20–25 в год, а во всей соседней подборке из 74 квартир дороже миллиарда стоят шесть, из них четыре — в одном «Брюсове». Предложение при этом растёт быстрее спроса. Срок реализации будет длинным, и он должен быть в ожиданиях с самого начала.', { bold: true })],
        [txt('Почти все соседи дешевле и все готовы раньше. '), txt('«Никитский-6» в ста пятидесяти метрах — 3,19 млн ₽ за метр с чистовой отделкой и сдачей в 2026 году. «Фамильный дом Люче» от MR Group в двухстах метрах — 2,94 млн ₽ с отделкой, 26 квартир в продаже. «Кло 17» — 2,63 млн ₽ без отделки. Единственное исключение по цене — «Брюсов», но и он сдаётся на три года раньше.', { bold: true })],
        [txt('И главное ограничение самой справки. '), txt('Цены конкурентов взяты из выгрузок и достоверны, а цена самого EDEN построена на одной опубликованной цифре старта продаж: поквартирного прайса нет, объявлений на площадках нет. Как появится реальный прайс, всё помеченное словом «расчёт» нужно пересчитать.', { bold: true })],
      ]),
