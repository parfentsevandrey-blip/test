...
Премиум-ремонт в нашем лоте сделан, доплачивать нечего. У соседей, где
экспозиция в бетоне, к цене метра добавлено 750 тыс. ₽/м² — ровно та сумма,
в которую ремонт обошёлся владельцу. Это и есть проверка суммы на прочность.
...
D = json.load(open('zl/zl_tables.json'))
FIT, OUR = D['fit'], D['our']
ROWS = [('НАШ ЛОТ', OUR['ppm'], 0, '87,5 м², 4 этаж · премиум-ремонт сделан')]
SHORT = {'Клубный дом Космо 4/22': '«Космо 4/22»', 'Клубный дом DUO': 'Клубный дом DUO',
         'Дом Лаврушинский': '«Дом Лаврушинский»', 'Софийский': '«Софийский» — апартаменты'}
for n, (b, f, sub) in D['ready'].items():
    if n == 'Золотой': continue
    ROWS.append((SHORT.get(n, n), b, f, sub))
ROWS.sort(key=lambda r: -(r[1] + r[2]))

img = Image.new('RGB', (W, H), SURFACE); dr = ImageDraw.Draw(img)
L, R, T = 440 * S, 210 * S, 178 * S
B = H - 96 * S
PW, MAXV = W - L - R, 4_300_000
...
dr.text((44 * S, 62 * S), 'Премиум-ремонт в нашем лоте сделан — доплачивать нечего. Там, где соседи продают бетон, к цене метра', font=fnt(17 * S), fill=MUTED)
dr.text((44 * S, 88 * S), 'добавлены 750 тыс. ₽/м² — смета владельца. Сопоставимая выборка 65–115 м², Циан, 31.07.2026.', font=fnt(17 * S), fill=MUTED)

lx, ly = 44 * S, 128 * S
for lab, c in [('Цена квартиры', C_FLAT), (f'Ремонт по смете владельца ({FIT // 1000} тыс. ₽/м²)', C_FIT)]:
...
img.save('zl/zl_chart.png', quality=95); print('saved', img.size, 643 / (W / H))
