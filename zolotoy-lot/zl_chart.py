...
В нашем лоте отделка выполнена, доплачивать нечего. У соседей, где экспозиция
идёт в бетоне, к цене метра добавлено 750 тыс. ₽/м² — отделка премиум-уровня
в де-люксе на Софийской набережной.
...
FIT, OUR = D['fit'], D['our']
ROWS = [('НАШ ЛОТ', OUR['ppm'], 0, '87,5 м², 4 этаж · отделка выполнена')]
...
dr.text((44 * S, 62 * S), 'В нашем лоте отделка выполнена — доплачивать нечего. Там, где соседи продают бетон, к цене метра', font=fnt(17 * S), fill=MUTED)
dr.text((44 * S, 88 * S), 'добавлены 750 тыс. ₽/м². Сопоставимая выборка 65–115 м², Циан, 31.07.2026.', font=fnt(17 * S), fill=MUTED)

lx, ly = 44 * S, 128 * S
for lab, c in [('Цена квартиры', C_FLAT), (f'Отделка ({FIT // 1000} тыс. ₽/м²)', C_FIT)]:
